    ...
    unsafe {
        {
            let mut init =
                cmd_entry{name:
                              b"paste-buffer\x00" as *const u8 as
                                  *const libc::c_char,
                          alias:
                              b"pasteb\x00" as *const u8 as
                                  *const libc::c_char,
                          args:
                              {
                                  let mut init =
...
unsafe extern "C" fn cmd_paste_buffer_exec(mut self_0: *mut cmd,
 ...
 -> cmd_retval {
    ...
    if !pb.is_null() && !(*wp).flags & 0x40 as libc::c_int != 0 {
        ...
        loop  {
            line =
                memchr(bufdata as *const libc::c_void, '\n' as i32,
                       bufend.wrapping_offset_from(bufdata) as libc::c_long as
                           libc::c_ulong) as *const libc::c_char;
            if line.is_null() { break ; }
            bufferevent_write((*wp).event, bufdata as *const libc::c_void,
                              line.wrapping_offset_from(bufdata) as
        ...
        if bufdata != bufend {
            bufferevent_write((*wp).event, bufdata as *const libc::c_void,
                              bufend.wrapping_offset_from(bufdata) as
                                  libc::c_long as size_t);
        }
        if bracket != 0 && (*(*wp).screen).mode & 0x400 as libc::c_int != 0 {
            bufferevent_write((*wp).event,
