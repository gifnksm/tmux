    ...
    unsafe {
        {
            let mut init =
                cmd_entry{name:
                              b"list-panes\x00" as *const u8 as
                                  *const libc::c_char,
                          alias:
                              b"lsp\x00" as *const u8 as *const libc::c_char,
                          args:
                              {
                                  let mut init =
                          ...
                          usage:
                              b"[-as] [-F format] [-f filter] [-t target-window]\x00"
 ...
 */
unsafe extern "C" fn cmd_list_panes_exec(mut self_0: *mut cmd,
 ...
 -> cmd_retval {
    ...
    if args_has(args, 'a' as i32 as u_char) != 0 {
        ...
        cmd_list_panes_session(self_0, s, item, 1 as libc::c_int);
    } else { cmd_list_panes_window(self_0, s, wl, item, 0 as libc::c_int); }
    return CMD_RETURN_NORMAL;
}
unsafe extern "C" fn cmd_list_panes_server(mut self_0: *mut cmd,
                                           mut item: *mut cmdq_item) {
    let mut s: *mut session = 0 as *mut session;
    s = sessions_RB_MINMAX(&mut sessions, -(1 as libc::c_int));
    while !s.is_null() {
        cmd_list_panes_session(self_0, s, item, 2 as libc::c_int);
        s = sessions_RB_NEXT(s)
    };
}
unsafe extern "C" fn cmd_list_panes_session(mut self_0: *mut cmd,
                                            mut s: *mut session,
                                            mut item: *mut cmdq_item,
...
unsafe extern "C" fn cmd_list_panes_window(mut self_0: *mut cmd,
                                           mut s: *mut session,
                                           mut wl: *mut winlink,
                                           mut item: *mut cmdq_item,
                                           mut type_0: libc::c_int) {
    ...
    if template.is_null() {
        match type_0 {
            0 => {
                template =
                    b"#{pane_index}: [#{pane_width}x#{pane_height}] [history #{history_size}/#{history_limit}, #{history_bytes} bytes] #{pane_id}#{?pane_active, (active),}#{?pane_dead, (dead),}\x00"
                        as *const u8 as *const libc::c_char
            }
            1 => {
                template =
                    b"#{window_index}.#{pane_index}: [#{pane_width}x#{pane_height}] [history #{history_size}/#{history_limit}, #{history_bytes} bytes] #{pane_id}#{?pane_active, (active),}#{?pane_dead, (dead),}\x00"
                        as *const u8 as *const libc::c_char
            }
            2 => {
                template =
                    b"#{session_name}:#{window_index}.#{pane_index}: [#{pane_width}x#{pane_height}] [history #{history_size}/#{history_limit}, #{history_bytes} bytes] #{pane_id}#{?pane_active, (active),}#{?pane_dead, (dead),}\x00"
                        as *const u8 as *const libc::c_char
            }
            _ => { }
        }
    ...
    while !wp.is_null() {
        ft =
            format_create(cmdq_get_client(item), item, 0 as libc::c_int,
                          0 as libc::c_int);
        format_add(ft, b"line\x00" as *const u8 as *const libc::c_char,
