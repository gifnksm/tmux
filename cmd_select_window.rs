    ...
    unsafe {
        {
            let mut init =
                cmd_entry{name:
                          ...
                          usage:
                              b"[-lnpT] [-t target-window]\x00" as *const u8
                          ...
                          exec:
                              Some(cmd_select_window_exec as
    ...
    unsafe {
        {
            let mut init =
                cmd_entry{name:
                              b"next-window\x00" as *const u8 as
                                  *const libc::c_char,
                          alias:
    ...
    unsafe {
        {
            let mut init =
                cmd_entry{name:
                              b"last-window\x00" as *const u8 as
                                  *const libc::c_char,
                          alias:
...
unsafe extern "C" fn cmd_select_window_exec(mut self_0: *mut cmd,
 ...
 -> cmd_retval {
    ...
    next =
        (cmd_get_entry(self_0) == &cmd_next_window_entry as *const cmd_entry)
            as libc::c_int;
    if args_has(args, 'n' as i32 as u_char) != 0 { next = 1 as libc::c_int }
    previous =
        (cmd_get_entry(self_0) ==
             &cmd_previous_window_entry as *const cmd_entry) as libc::c_int;
    if args_has(args, 'p' as i32 as u_char) != 0 {
        previous = 1 as libc::c_int
    }
    last =
        (cmd_get_entry(self_0) == &cmd_last_window_entry as *const cmd_entry)
            as libc::c_int;
    if args_has(args, 'l' as i32 as u_char) != 0 { last = 1 as libc::c_int }
