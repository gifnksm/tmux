...
extern "C" {
    ...
    #[no_mangle]
    fn screen_alternate_off(_: *mut screen, _: *mut grid_cell,
                            _: libc::c_int);
    #[no_mangle]
...
unsafe extern "C" fn screen_write_offset_timer(mut fd: libc::c_int,
...
/* Set cursor position. */
unsafe extern "C" fn screen_write_set_cursor(mut ctx: *mut screen_write_ctx,
                                             mut cx: libc::c_int,
                                             mut cy: libc::c_int) {
    ...
    if cx != -(1 as libc::c_int) && cx as u_int == (*s).cx &&
           cy != -(1 as libc::c_int) && cy as u_int == (*s).cy {
        return
    }
    ...
    if event_initialized(&mut (*w).offset_timer) == 0 {
        event_set(&mut (*w).offset_timer, -(1 as libc::c_int),
                  0 as libc::c_int as libc::c_short,
                  Some(screen_write_offset_timer as
                           unsafe extern "C" fn(_: libc::c_int,
                                                _: libc::c_short,
                                                _: *mut libc::c_void) -> ()),
                  w as *mut libc::c_void);
    }
    if event_pending(&mut (*w).offset_timer,
...
unsafe extern "C" fn screen_write_redraw_cb(mut ttyctx: *const tty_ctx) {
    ...
    if !wp.is_null() { (*wp).flags |= 0x1 as libc::c_int };
}
/* Update context for client. */
unsafe extern "C" fn screen_write_set_client_cb(mut ttyctx: *mut tty_ctx,
                                                mut c: *mut client)
 -> libc::c_int {
    let mut wp: *mut window_pane = (*ttyctx).arg as *mut window_pane;
    ...
    if (*wp).layout_cell.is_null() { return 0 as libc::c_int }
    ...
    if (*c).flags & 0x20000000 as libc::c_int as libc::c_ulong != 0 {
		 ...
		 */
        log_debug(b"adding %%%u to deferred redraw\x00" as *const u8 as
                      *const libc::c_char, (*wp).id);
        (*wp).flags |= 0x1 as libc::c_int;
        return -(1 as libc::c_int)
    }
    (*ttyctx).bigger =
        tty_window_offset(&mut (*c).tty, &mut (*ttyctx).wox,
                          &mut (*ttyctx).woy, &mut (*ttyctx).wsx,
                          &mut (*ttyctx).wsy);
    ...
    if status_at_line(c) == 0 as libc::c_int {
        (*ttyctx).yoff =
            ((*ttyctx).yoff as libc::c_uint).wrapping_add(status_line_size(c))
                as u_int as u_int
    }
    return 1 as libc::c_int;
}
...
unsafe extern "C" fn screen_write_initctx(mut ctx: *mut screen_write_ctx,
    ...
    if (*ctx).init_ctx_cb.is_some() {
        ...
        (*ttyctx).redraw_cb =
            Some(screen_write_redraw_cb as
                     unsafe extern "C" fn(_: *const tty_ctx) -> ());
        if (*ctx).wp.is_null() {
            (*ttyctx).set_client_cb = None
        } else {
            (*ttyctx).set_client_cb =
                Some(screen_write_set_client_cb as
                         unsafe extern "C" fn(_: *mut tty_ctx, _: *mut client)
                             -> libc::c_int)
        }
        (*ttyctx).arg = (*ctx).wp as *mut libc::c_void
    }
    if !(*ctx).wp.is_null() && !(*ctx).flags & 0x1 as libc::c_int != 0 &&
           (sync != 0 || (*ctx).wp != (*(*(*ctx).wp).window).active) {
        tty_write(Some(tty_cmd_syncstart as
                           unsafe extern "C" fn(_: *mut tty,
                                                _: *const tty_ctx) -> ()),
                  ttyctx);
...
/* Set up for writing. */
unsafe extern "C" fn screen_write_init(mut ctx: *mut screen_write_ctx,
                                       mut s: *mut screen) {
    memset(ctx as *mut libc::c_void, 0 as libc::c_int,
           ::std::mem::size_of::<screen_write_ctx>() as libc::c_ulong);
    (*ctx).s = s;
    if (*(*ctx).s).write_list.is_null() { screen_write_make_list((*ctx).s); }
    (*ctx).item =
...
#[no_mangle]
pub unsafe extern "C" fn screen_write_start_pane(mut ctx:
                                                     *mut screen_write_ctx,
                                                 mut wp: *mut window_pane,
                                                 mut s: *mut screen) {
    if s.is_null() { s = (*wp).screen }
    screen_write_init(ctx, s);
    (*ctx).wp = wp;
    if log_get_level() != 0 as libc::c_int {
        log_debug(b"%s: size %ux%u, pane %%%u (at %u,%u)\x00" as *const u8 as
                      *const libc::c_char,
                  (*::std::mem::transmute::<&[u8; 24],
                                            &[libc::c_char; 24]>(b"screen_write_start_pane\x00")).as_ptr(),
                  (*(*(*ctx).s).grid).sx, (*(*(*ctx).s).grid).sy, (*wp).id,
                  (*wp).xoff, (*wp).yoff);
    };
}
/* Initialize writing with a callback. */
#[no_mangle]
pub unsafe extern "C" fn screen_write_start_callback(mut ctx:
                                                         *mut screen_write_ctx,
                                                     mut s: *mut screen,
                                                     mut cb:
    ...
    (*ctx).arg = arg;
    if log_get_level() != 0 as libc::c_int {
        log_debug(b"%s: size %ux%u, with callback\x00" as *const u8 as
                      *const libc::c_char,
                  (*::std::mem::transmute::<&[u8; 28],
                                            &[libc::c_char; 28]>(b"screen_write_start_callback\x00")).as_ptr(),
...
pub unsafe extern "C" fn screen_write_start(mut ctx: *mut screen_write_ctx,
    ...
    screen_write_init(ctx, s);
    if log_get_level() != 0 as libc::c_int {
        log_debug(b"%s: size %ux%u, no pane\x00" as *const u8 as
                      *const libc::c_char,
...
pub unsafe extern "C" fn screen_write_stop(mut ctx: *mut screen_write_ctx) {
    ...
    if !(*ctx).wp.is_null() {
        (*(*ctx).wp).written =
            ((*(*ctx).wp).written as
                 libc::c_ulong).wrapping_add((*ctx).written as libc::c_ulong)
                as size_t as size_t;
        (*(*ctx).wp).skipped =
            ((*(*ctx).wp).skipped as
                 libc::c_ulong).wrapping_add((*ctx).skipped as libc::c_ulong)
                as size_t as size_t
    }
...
pub unsafe extern "C" fn screen_write_reset(mut ctx: *mut screen_write_ctx) {
    let mut s: *mut screen = (*ctx).s;
    screen_reset_tabs(s);
    screen_write_scrollregion(ctx, 0 as libc::c_int as u_int,
...
#[no_mangle]
pub unsafe extern "C" fn screen_write_strlen(mut fmt: *const libc::c_char,
                                             mut args: ...) -> size_t {
    ...
    while *ptr as libc::c_int != '\u{0}' as i32 {
        if *ptr as libc::c_int > 0x7f as libc::c_int &&
            ...
            if more as libc::c_uint ==
                ...
                size =
                    (size as
                         libc::c_ulong).wrapping_add(ud.width as
                                                         libc::c_ulong) as
                        size_t as size_t
            }
        } else {
...
#[no_mangle]
pub unsafe extern "C" fn screen_write_text(mut ctx: *mut screen_write_ctx,
                                           mut cx: u_int, mut width: u_int,
                                           mut lines: u_int,
                                           mut more: libc::c_int,
                                           mut gcp: *const grid_cell,
    ...
    loop  {
        ...
        while (*text.offset(end as isize)).size as libc::c_int !=
                  0 as libc::c_int {
            if (*text.offset(end as isize)).size as libc::c_int ==
                   1 as libc::c_int &&
                   (*text.offset(end as
            ...
            if at.wrapping_add((*text.offset(end as isize)).width as
                                   libc::c_uint) > left {
                break ;
            }
            at =
                (at as
                     libc::c_uint).wrapping_add((*text.offset(end as
                                                                  isize)).width
        ...
        if (*text.offset(end as isize)).size as libc::c_int ==
               0 as libc::c_int {
            next = end
        } else if (*text.offset(end as isize)).size as libc::c_int ==
                      1 as libc::c_int &&
                      (*text.offset(end as
            ...
            i = end;
            while i > idx {
                if (*text.offset(i as isize)).size as libc::c_int ==
                       1 as libc::c_int &&
                       (*text.offset(i as
                                         isize)).data[0 as libc::c_int as
                                                          usize] as
        ...
        idx = next;
        if (*s).cy ==
               cy.wrapping_add(lines).wrapping_sub(1 as libc::c_int as
                                                       libc::c_uint) ||
               (*text.offset(idx as isize)).size as libc::c_int ==
                   0 as libc::c_int {
            break ;
        }
...
#[no_mangle]
pub unsafe extern "C" fn screen_write_puts(mut ctx: *mut screen_write_ctx,
                                           mut gcp: *const grid_cell,
                                           mut fmt: *const libc::c_char,
                                           mut args: ...) {
    let mut ap: ::std::ffi::VaListImpl;
    ap = args.clone();
    screen_write_vnputs(ctx, -(1 as libc::c_int) as ssize_t, gcp, fmt,
                        ap.as_va_list());
}
/* Write string with length limit (-1 for unlimited). */
#[no_mangle]
pub unsafe extern "C" fn screen_write_nputs(mut ctx: *mut screen_write_ctx,
                                            mut maxlen: ssize_t,
                                            mut gcp: *const grid_cell,
                                            mut fmt: *const libc::c_char,
                                            mut args: ...) {
    ...
    while *ptr as libc::c_int != '\u{0}' as i32 {
        if *ptr as libc::c_int > 0x7f as libc::c_int &&
            ...
            if *ptr as libc::c_int == '\u{1}' as i32 {
                gc.attr =
                    (gc.attr as libc::c_int ^ 0x80 as libc::c_int) as u_short
            } else if *ptr as libc::c_int == '\n' as i32 {
                screen_write_linefeed(ctx, 0 as libc::c_int,
...
pub unsafe extern "C" fn screen_write_fast_copy(mut ctx:
    ...
    while yy < py.wrapping_add(ny) {
        ...
        while xx < px.wrapping_add(nx) {
            if xx >= (*grid_get_line(gd, yy)).cellsize { break ; }
...
pub unsafe extern "C" fn screen_write_hline(mut ctx: *mut screen_write_ctx,
    ...
    screen_write_putc(ctx, &mut gc,
                      if left != 0 { 't' as i32 } else { 'q' as i32 } as
                          u_char);
    ...
    screen_write_putc(ctx, &mut gc,
                      if right != 0 { 'u' as i32 } else { 'q' as i32 } as
                          u_char);
...
pub unsafe extern "C" fn screen_write_vline(mut ctx: *mut screen_write_ctx,
    ...
    gc.attr = (gc.attr as libc::c_int | 0x80 as libc::c_int) as u_short;
    screen_write_putc(ctx, &mut gc,
                      if top != 0 { 'w' as i32 } else { 'x' as i32 } as
                          u_char);
    i = 1 as libc::c_int as u_int;
    while i < ny.wrapping_sub(1 as libc::c_int as libc::c_uint) {
        screen_write_set_cursor(ctx, cx as libc::c_int,
                                cy.wrapping_add(i) as libc::c_int);
        screen_write_putc(ctx, &mut gc, 'x' as i32 as u_char);
        i = i.wrapping_add(1)
    }
    screen_write_set_cursor(ctx, cx as libc::c_int,
                            cy.wrapping_add(ny).wrapping_sub(1 as libc::c_int
                                                                 as
                                                                 libc::c_uint)
                                as libc::c_int);
    screen_write_putc(ctx, &mut gc,
                      if bottom != 0 { 'v' as i32 } else { 'x' as i32 } as
                          u_char);
    screen_write_set_cursor(ctx, cx as libc::c_int, cy as libc::c_int);
}
/* Draw a menu on screen. */
#[no_mangle]
pub unsafe extern "C" fn screen_write_menu(mut ctx: *mut screen_write_ctx,
                                           mut menu: *mut menu,
                                           mut choice: libc::c_int,
                                           mut choice_gc: *const grid_cell) {
    let mut s: *mut screen = (*ctx).s;
    let mut default_gc: grid_cell =
        grid_cell{data: utf8_data{data: [0; 21], have: 0, size: 0, width: 0,},
                  attr: 0,
                  flags: 0,
                  fg: 0,
    ...
    cx = (*s).cx;
    cy = (*s).cy;
    memcpy(&mut default_gc as *mut grid_cell as *mut libc::c_void,
           &grid_default_cell as *const grid_cell as *const libc::c_void,
           ::std::mem::size_of::<grid_cell>() as libc::c_ulong);
    screen_write_box(ctx,
                     (*menu).width.wrapping_add(4 as libc::c_int as
                                                    libc::c_uint),
                     (*menu).count.wrapping_add(2 as libc::c_int as
                                                    libc::c_uint));
    screen_write_cursormove(ctx,
                            cx.wrapping_add(2 as libc::c_int as libc::c_uint)
                                as libc::c_int, cy as libc::c_int,
                            0 as libc::c_int);
    format_draw(ctx, &mut default_gc, (*menu).width, (*menu).title,
    ...
    while i < (*menu).count {
        ...
        if name.is_null() {
            screen_write_cursormove(ctx, cx as libc::c_int,
                                    cy.wrapping_add(1 as libc::c_int as
                                                        libc::c_uint).wrapping_add(i)
                                        as libc::c_int, 0 as libc::c_int);
            screen_write_hline(ctx,
                               (*menu).width.wrapping_add(4 as libc::c_int as
                                                              libc::c_uint),
                               1 as libc::c_int, 1 as libc::c_int);
        } else {
            if choice >= 0 as libc::c_int && i == choice as u_int &&
                   *name as libc::c_int != '-' as i32 {
                gc = choice_gc
            }
            screen_write_cursormove(ctx,
                                    cx.wrapping_add(2 as libc::c_int as
                                                        libc::c_uint) as
                                        libc::c_int,
                                    cy.wrapping_add(1 as libc::c_int as
                                                        libc::c_uint).wrapping_add(i)
            ...
            if *name as libc::c_int == '-' as i32 {
                name = name.offset(1);
                default_gc.attr =
                    (default_gc.attr as libc::c_int | 0x2 as libc::c_int) as
                        u_short;
                format_draw(ctx, gc, (*menu).width, name,
                            0 as *mut style_ranges);
                default_gc.attr =
                    (default_gc.attr as libc::c_int & !(0x2 as libc::c_int))
                        as u_short
            } else {
                format_draw(ctx, gc, (*menu).width, name,
                            0 as *mut style_ranges);
...
pub unsafe extern "C" fn screen_write_box(mut ctx: *mut screen_write_ctx,
    ...
    i = 1 as libc::c_int as u_int;
    while i < ny.wrapping_sub(1 as libc::c_int as libc::c_uint) {
        screen_write_set_cursor(ctx,
                                cx.wrapping_add(nx).wrapping_sub(1 as
                                                                     libc::c_int
                                                                     as
                                                                     libc::c_uint)
                                    as libc::c_int,
...
pub unsafe extern "C" fn screen_write_preview(mut ctx: *mut screen_write_ctx,
    ...
    if (*src).mode & 0x1 as libc::c_int != 0 {
        ...
        if px < nx.wrapping_div(3 as libc::c_int as libc::c_uint) {
            ...
            px =
                px.wrapping_sub(nx.wrapping_div(3 as libc::c_int as
                                                    libc::c_uint))
        }
        ...
        if py < ny.wrapping_div(3 as libc::c_int as libc::c_uint) {
            ...
            py =
                py.wrapping_sub(ny.wrapping_div(3 as libc::c_int as
                                                    libc::c_uint))
        }
    ...
    } else { px = 0 as libc::c_int as u_int; py = 0 as libc::c_int as u_int }
    ...
    if (*src).mode & 0x1 as libc::c_int != 0 {
        ...
        screen_write_set_cursor(ctx,
                                cx.wrapping_add((*src).cx.wrapping_sub(px)) as
                                    libc::c_int,
                                cy.wrapping_add((*src).cy.wrapping_sub(py)) as
                                    libc::c_int);
        screen_write_cell(ctx, &mut gc);
    };
}
...
pub unsafe extern "C" fn screen_write_cursorup(mut ctx: *mut screen_write_ctx,
    ...
    if cx == (*(*s).grid).sx { cx = cx.wrapping_sub(1) }
...
pub unsafe extern "C" fn screen_write_cursordown(mut ctx:
    ...
    } else if ny == 0 as libc::c_int as libc::c_uint { return }
...
pub unsafe extern "C" fn screen_write_backspace(mut ctx:
    ...
    if cx == 0 as libc::c_int as libc::c_uint {
        if cy == 0 as libc::c_int as libc::c_uint { return }
        gl =
            grid_get_line((*s).grid,
                          (*(*s).grid).hsize.wrapping_add(cy).wrapping_sub(1
                                                                               as
                                                                               libc::c_int
                                                                               as
                                                                               libc::c_uint));
...
pub unsafe extern "C" fn screen_write_alignmenttest(mut ctx:
    ...
    screen_write_collect_clear(ctx, 0 as libc::c_int as u_int,
                               (*(*s).grid).sy.wrapping_sub(1 as libc::c_int
                                                                as
                                                                libc::c_uint));
    tty_write(Some(tty_cmd_alignmenttest as
                       unsafe extern "C" fn(_: *mut tty, _: *const tty_ctx)
                           -> ()), &mut ttyctx);
}
...
pub unsafe extern "C" fn screen_write_insertcharacter(mut ctx:
    ...
    screen_write_collect_flush(ctx, 0 as libc::c_int,
                               (*::std::mem::transmute::<&[u8; 29],
                                                         &[libc::c_char; 29]>(b"screen_write_insertcharacter\x00")).as_ptr());
    ttyctx.num = nx;
    tty_write(Some(tty_cmd_insertcharacter as
                       unsafe extern "C" fn(_: *mut tty, _: *const tty_ctx)
                           -> ()), &mut ttyctx);
}
...
pub unsafe extern "C" fn screen_write_deletecharacter(mut ctx:
    ...
    screen_write_collect_flush(ctx, 0 as libc::c_int,
                               (*::std::mem::transmute::<&[u8; 29],
                                                         &[libc::c_char; 29]>(b"screen_write_deletecharacter\x00")).as_ptr());
    ttyctx.num = nx;
    tty_write(Some(tty_cmd_deletecharacter as
                       unsafe extern "C" fn(_: *mut tty, _: *const tty_ctx)
                           -> ()), &mut ttyctx);
}
/* Clear nx characters. */
...
pub unsafe extern "C" fn screen_write_clearcharacter(mut ctx:
    ...
    screen_write_initctx(ctx, &mut ttyctx, 0 as libc::c_int);
    ttyctx.bg = bg;
    grid_view_clear((*s).grid, (*s).cx, (*s).cy, nx,
                    1 as libc::c_int as u_int, bg);
    screen_write_collect_flush(ctx, 0 as libc::c_int,
                               (*::std::mem::transmute::<&[u8; 28],
                                                         &[libc::c_char; 28]>(b"screen_write_clearcharacter\x00")).as_ptr());
    ttyctx.num = nx;
    tty_write(Some(tty_cmd_clearcharacter as
                       unsafe extern "C" fn(_: *mut tty, _: *const tty_ctx)
                           -> ()), &mut ttyctx);
}
/* Insert ny lines. */
...
pub unsafe extern "C" fn screen_write_insertline(mut ctx:
    ...
    if (*s).cy < (*s).rupper || (*s).cy > (*s).rlower {
        ...
        screen_write_collect_flush(ctx, 0 as libc::c_int,
                                   (*::std::mem::transmute::<&[u8; 24],
                                                             &[libc::c_char; 24]>(b"screen_write_insertline\x00")).as_ptr());
        ttyctx.num = ny;
        tty_write(Some(tty_cmd_insertline as
                           unsafe extern "C" fn(_: *mut tty,
                                                _: *const tty_ctx) -> ()),
                  &mut ttyctx);
        return
    ...
    if ny >
           (*s).rlower.wrapping_add(1 as libc::c_int as
...
pub unsafe extern "C" fn screen_write_deleteline(mut ctx:
    ...
    if (*s).cy < (*s).rupper || (*s).cy > (*s).rlower {
        ...
        screen_write_collect_flush(ctx, 0 as libc::c_int,
                                   (*::std::mem::transmute::<&[u8; 24],
                                                             &[libc::c_char; 24]>(b"screen_write_deleteline\x00")).as_ptr());
        ttyctx.num = ny;
        tty_write(Some(tty_cmd_deleteline as
                           unsafe extern "C" fn(_: *mut tty,
                                                _: *const tty_ctx) -> ()),
                  &mut ttyctx);
        return
...
pub unsafe extern "C" fn screen_write_clearline(mut ctx:
    ...
    grid_view_clear((*s).grid, 0 as libc::c_int as u_int, (*s).cy, sx,
                    1 as libc::c_int as u_int, bg);
...
pub unsafe extern "C" fn screen_write_clearendofline(mut ctx:
    ...
    (*ci).entry.tqe_prev =
        (*(*(*ctx).s).write_list.offset((*s).cy as isize)).items.tqh_last;
    let ref mut fresh2 =
        *(*(*(*ctx).s).write_list.offset((*s).cy as isize)).items.tqh_last;
    *fresh2 = ci;
    let ref mut fresh3 =
...
pub unsafe extern "C" fn screen_write_clearstartofline(mut ctx:
    ...
    if (*s).cx > sx.wrapping_sub(1 as libc::c_int as libc::c_uint) {
        ...
        grid_view_clear((*s).grid, 0 as libc::c_int as u_int, (*s).cy,
...
pub unsafe extern "C" fn screen_write_cursormove(mut ctx:
    ...
    let mut s: *mut screen = (*ctx).s;
    if origin != 0 && py != -(1 as libc::c_int) &&
           (*s).mode & 0x2000 as libc::c_int != 0 {
        if py as u_int > (*s).rlower.wrapping_sub((*s).rupper) {
            py = (*s).rlower as libc::c_int
        } else {
            py =
                (py as libc::c_uint).wrapping_add((*s).rupper) as libc::c_int
                    as libc::c_int
        }
    }
    if px != -(1 as libc::c_int) &&
       ...
       {
        px =
            (*(*s).grid).sx.wrapping_sub(1 as libc::c_int as libc::c_uint) as
                libc::c_int
    }
    if py != -(1 as libc::c_int) &&
       ...
       {
        py =
            (*(*s).grid).sy.wrapping_sub(1 as libc::c_int as libc::c_uint) as
                libc::c_int
    }
    ...
    if (*s).cy == (*s).rupper {
        ...
        screen_write_collect_flush(ctx, 0 as libc::c_int,
                                   (*::std::mem::transmute::<&[u8; 26],
                                                             &[libc::c_char; 26]>(b"screen_write_reverseindex\x00")).as_ptr());
        screen_write_initctx(ctx, &mut ttyctx, 1 as libc::c_int);
        ttyctx.bg = bg;
        tty_write(Some(tty_cmd_reverseindex as
                           unsafe extern "C" fn(_: *mut tty,
                                                _: *const tty_ctx) -> ()),
                  &mut ttyctx);
    } else if (*s).cy > 0 as libc::c_int as libc::c_uint {
        screen_write_set_cursor(ctx, -(1 as libc::c_int),
                                (*s).cy.wrapping_sub(1 as libc::c_int as
                                                         libc::c_uint) as
                                    libc::c_int);
    };
}
...
pub unsafe extern "C" fn screen_write_scrollregion(mut ctx:
                                                   ...
                                                   mut rlower: u_int) {
    let mut s: *mut screen = (*ctx).s;
    if rupper > (*(*s).grid).sy.wrapping_sub(1 as libc::c_int as libc::c_uint)
       {
        rupper =
            (*(*s).grid).sy.wrapping_sub(1 as libc::c_int as libc::c_uint)
    }
    if rlower > (*(*s).grid).sy.wrapping_sub(1 as libc::c_int as libc::c_uint)
       {
        rlower =
            (*(*s).grid).sy.wrapping_sub(1 as libc::c_int as libc::c_uint)
    }
    ...
    screen_write_collect_flush(ctx, 0 as libc::c_int,
                               (*::std::mem::transmute::<&[u8; 26],
                                                         &[libc::c_char; 26]>(b"screen_write_scrollregion\x00")).as_ptr());
...
pub unsafe extern "C" fn screen_write_linefeed(mut ctx: *mut screen_write_ctx,
    ...
    } else { (*gl).flags &= !(0x1 as libc::c_int) }
...
pub unsafe extern "C" fn screen_write_scrollup(mut ctx: *mut screen_write_ctx,
    ...
    if lines == 0 as libc::c_int as libc::c_uint {
        lines = 1 as libc::c_int as u_int
                  ...
                  (*s).rlower.wrapping_sub((*s).rupper).wrapping_add(1 as
                                                                         libc::c_int
                                                                         as
                                                                         libc::c_uint)
     {
    ...
    if bg != (*ctx).bg {
        screen_write_collect_flush(ctx, 1 as libc::c_int,
                                   (*::std::mem::transmute::<&[u8; 22],
                                                             &[libc::c_char; 22]>(b"screen_write_scrollup\x00")).as_ptr());
        (*ctx).bg = bg
    }
    i = 0 as libc::c_int as u_int;
    while i < lines {
...
/* Scroll down. */
#[no_mangle]
pub unsafe extern "C" fn screen_write_scrolldown(mut ctx:
                                                     *mut screen_write_ctx,
                                                 mut lines: u_int,
                                                 mut bg: u_int) {
    let mut s: *mut screen = (*ctx).s;
    ...
    screen_write_collect_flush(ctx, 0 as libc::c_int,
                               (*::std::mem::transmute::<&[u8; 24],
                                                         &[libc::c_char; 24]>(b"screen_write_scrolldown\x00")).as_ptr());
    ttyctx.num = lines;
    tty_write(Some(tty_cmd_scrolldown as
                       unsafe extern "C" fn(_: *mut tty, _: *const tty_ctx)
                           -> ()), &mut ttyctx);
}
...
pub unsafe extern "C" fn screen_write_clearendofscreen(mut ctx:
    ...
    if (*s).cx == 0 as libc::c_int as libc::c_uint &&
        ...
        grid_view_clear(gd, 0 as libc::c_int as u_int,
                        (*s).cy.wrapping_add(1 as libc::c_int as
                                                 libc::c_uint), sx,
                        sy.wrapping_sub((*s).cy.wrapping_add(1 as libc::c_int
                                                                 as
                                                                 libc::c_uint)),
                        bg);
    }
    screen_write_collect_clear(ctx,
                               (*s).cy.wrapping_add(1 as libc::c_int as
                                                        libc::c_uint),
                               sy.wrapping_sub((*s).cy.wrapping_add(1 as
                                                                        libc::c_int
                                                                        as
                                                                        libc::c_uint)));
    screen_write_collect_flush(ctx, 0 as libc::c_int,
                               (*::std::mem::transmute::<&[u8; 30],
                                                         &[libc::c_char; 30]>(b"screen_write_clearendofscreen\x00")).as_ptr());
    tty_write(Some(tty_cmd_clearendofscreen as
                       unsafe extern "C" fn(_: *mut tty, _: *const tty_ctx)
                           -> ()), &mut ttyctx);
}
...
pub unsafe extern "C" fn screen_write_clearstartofscreen(mut ctx:
    ...
    if (*s).cy > 0 as libc::c_int as libc::c_uint {
        grid_view_clear((*s).grid, 0 as libc::c_int as u_int,
                        0 as libc::c_int as u_int, sx, (*s).cy, bg);
    }
    if (*s).cx > sx.wrapping_sub(1 as libc::c_int as libc::c_uint) {
        grid_view_clear((*s).grid, 0 as libc::c_int as u_int, (*s).cy, sx,
                        1 as libc::c_int as u_int, bg);
    ...
    screen_write_collect_flush(ctx, 0 as libc::c_int,
                               (*::std::mem::transmute::<&[u8; 32],
                                                         &[libc::c_char; 32]>(b"screen_write_clearstartofscreen\x00")).as_ptr());
    tty_write(Some(tty_cmd_clearstartofscreen as
                       unsafe extern "C" fn(_: *mut tty, _: *const tty_ctx)
                           -> ()), &mut ttyctx);
}
/* Clear entire screen. */
...
pub unsafe extern "C" fn screen_write_clearscreen(mut ctx:
    ...
    if (*(*s).grid).flags & 0x1 as libc::c_int != 0 {
        ...
        grid_view_clear((*s).grid, 0 as libc::c_int as u_int,
                        0 as libc::c_int as u_int, sx, sy, bg);
    }
    screen_write_collect_clear(ctx, 0 as libc::c_int as u_int, sy);
    tty_write(Some(tty_cmd_clearscreen as
                       unsafe extern "C" fn(_: *mut tty, _: *const tty_ctx)
                           -> ()), &mut ttyctx);
}
...
unsafe extern "C" fn screen_write_collect_clear_start(mut ctx:
                                                          *mut screen_write_ctx,
                                                      mut y: u_int,
                                                      mut x: u_int) {
    ...
    if (*(*(*ctx).s).write_list.offset(y as isize)).items.tqh_first.is_null()
       {
        return
    }
    ...
    while !ci.is_null() &&
              { tmp = (*ci).entry.tqe_next; (1 as libc::c_int) != 0 } {
        match (*ci).type_0 as libc::c_uint {
            ...
            0 => {
                if (*ci).x > x {
                    current_block_15 = 17179679302217393232;
                } else { current_block_15 = 12209867499936983673; }
            }
            2 | _ => { current_block_15 = 12209867499936983673; }
        }
        match current_block_15 {
            12209867499936983673 => {
                ...
                if !(*ci).entry.tqe_next.is_null() {
                    ...
                    let ref mut fresh6 =
                        (*(*(*ctx).s).write_list.offset(y as
                                                            isize)).items.tqh_last;
    ...
    (*ctx).skipped =
        ((*ctx).skipped as libc::c_ulong).wrapping_add(size) as u_int as
            u_int;
    log_debug(b"%s: dropped %u items (%zu bytes) (line %u)\x00" as *const u8
                  as *const libc::c_char,
              (*::std::mem::transmute::<&[u8; 33],
                                        &[libc::c_char; 33]>(b"screen_write_collect_clear_start\x00")).as_ptr(),
              items, size, y);
}
/* Clear to end of a collected line. */
unsafe extern "C" fn screen_write_collect_clear_end(mut ctx:
                                                        *mut screen_write_ctx,
                                                    mut y: u_int,
                                                    mut x: u_int) {
    let mut ci: *mut screen_write_collect_item =
    ...
    while !ci.is_null() &&
        ...
        match (*ci).type_0 as libc::c_uint {
            ...
            1 | _ => { current_block_15 = 12209867499936983673; }
        }
        match current_block_15 {
            12209867499936983673 => {
                items = items.wrapping_add(1);
    ...
    log_debug(b"%s: dropped %u items (%zu bytes) (line %u)\x00" as *const u8
              ...
              (*::std::mem::transmute::<&[u8; 31],
                                        &[libc::c_char; 31]>(b"screen_write_collect_clear_end\x00")).as_ptr(),
              items, size, y);
}
/* $OpenBSD$ */
/*
...
unsafe extern "C" fn screen_write_collect_clear(mut ctx:
    ...
    while i < y.wrapping_add(n) {
        if !(*(*(*ctx).s).write_list.offset(i as
                                                isize)).items.tqh_first.is_null()
           {
            items = 0 as libc::c_int as u_int;
            size = 0 as libc::c_int as size_t;
            cl =
                &mut *(*(*ctx).s).write_list.offset(i as isize) as
            ...
            while !ci.is_null() &&
                ...
                if !(*ci).entry.tqe_next.is_null() {
                    (*(*ci).entry.tqe_next).entry.tqe_prev =
                        (*ci).entry.tqe_prev
                } else { (*cl).items.tqh_last = (*ci).entry.tqe_prev }
                *(*ci).entry.tqe_prev = (*ci).entry.tqe_next;
                free(ci as *mut libc::c_void);
                ci = tmp
            }
            (*ctx).skipped =
                ((*ctx).skipped as libc::c_ulong).wrapping_add(size) as u_int
                    as u_int;
            log_debug(b"%s: dropped %u items (%zu bytes) (line %u)\x00" as
...
unsafe extern "C" fn screen_write_collect_scroll(mut ctx:
    ...
    while y < (*s).rlower {
        cl =
            &mut *(*(*ctx).s).write_list.offset(y.wrapping_add(1 as
                                                                   libc::c_int
                                                                   as
                                                                   libc::c_uint)
                                                    as isize) as
        ...
        let ref mut fresh10 =
            (*(*(*ctx).s).write_list.offset(y as isize)).data;
        *fresh10 = (*cl).data;
        y = y.wrapping_add(1)
    }
    ...
    let ref mut fresh11 =
        (*(*(*ctx).s).write_list.offset((*s).rlower as isize)).data;
    *fresh11 = saved;
}
...
unsafe extern "C" fn screen_write_collect_flush(mut ctx:
    ...
    if (*ctx).scrolled != 0 as libc::c_int as libc::c_uint {
        ...
        ttyctx.bg = (*ctx).bg;
        tty_write(Some(tty_cmd_scrollup as
                           unsafe extern "C" fn(_: *mut tty,
                                                _: *const tty_ctx) -> ()),
                  &mut ttyctx);
    }
    ...
    while y < (*(*s).grid).sy {
        cl =
            &mut *(*(*ctx).s).write_list.offset(y as isize) as
                *mut screen_write_collect_line;
        if (*cl).bg != 0 as libc::c_int as libc::c_uint {
            screen_write_set_cursor(ctx, 0 as libc::c_int, y as libc::c_int);
            screen_write_initctx(ctx, &mut ttyctx, 1 as libc::c_int);
            ttyctx.bg =
                (*cl).bg.wrapping_sub(1 as libc::c_int as libc::c_uint);
            tty_write(Some(tty_cmd_clearline as
                               unsafe extern "C" fn(_: *mut tty,
                                                    _: *const tty_ctx) -> ()),
                      &mut ttyctx);
        }
        ...
        while !ci.is_null() &&
                  { tmp = (*ci).entry.tqe_next; (1 as libc::c_int) != 0 } {
            screen_write_set_cursor(ctx, (*ci).x as libc::c_int,
                                    y as libc::c_int);
            if (*ci).type_0 as libc::c_uint ==
                   CLEAR_END as libc::c_int as libc::c_uint {
                log_debug(b"XXX %u %u\x00" as *const u8 as
                              *const libc::c_char, (*ci).x, (*ci).bg);
                screen_write_initctx(ctx, &mut ttyctx, 1 as libc::c_int);
                ttyctx.bg = (*ci).bg;
                tty_write(Some(tty_cmd_clearendofline as
                                   unsafe extern "C" fn(_: *mut tty,
                                                        _: *const tty_ctx)
                                       -> ()), &mut ttyctx);
            } else if (*ci).type_0 as libc::c_uint ==
                          CLEAR_START as libc::c_int as libc::c_uint {
                screen_write_initctx(ctx, &mut ttyctx, 1 as libc::c_int);
                ttyctx.bg = (*ci).bg;
                tty_write(Some(tty_cmd_clearstartofline as
                                   unsafe extern "C" fn(_: *mut tty,
                                                        _: *const tty_ctx)
                                       -> ()), &mut ttyctx);
            } else {
                ...
                ttyctx.ptr =
                    (*cl).data.offset((*ci).x as isize) as *mut libc::c_void;
                ttyctx.num = (*ci).used;
                tty_write(Some(tty_cmd_cells as
                                   unsafe extern "C" fn(_: *mut tty,
                                                        _: *const tty_ctx)
                                       -> ()), &mut ttyctx);
            }
            ...
            written =
                (written as
                     libc::c_ulong).wrapping_add((*ci).used as libc::c_ulong)
                    as size_t as size_t;
            if !(*ci).entry.tqe_next.is_null() {
    ...
    (*s).cx = cx;
    (*s).cy = cy;
    log_debug(b"%s: flushed %u items (%zu bytes) (%s)\x00" as *const u8 as
                  *const libc::c_char,
              (*::std::mem::transmute::<&[u8; 27],
                                        &[libc::c_char; 27]>(b"screen_write_collect_flush\x00")).as_ptr(),
              items, written, from);
    (*ctx).written =
        ((*ctx).written as libc::c_ulong).wrapping_add(written) as u_int as
            u_int;
}
...
pub unsafe extern "C" fn screen_write_collect_end(mut ctx:
    ...
    let mut ci: *mut screen_write_collect_item = (*ctx).item;
    let mut cl: *mut screen_write_collect_line =
        &mut *(*s).write_list.offset((*s).cy as isize) as
            *mut screen_write_collect_line;
    let mut gc: grid_cell =
        grid_cell{data: utf8_data{data: [0; 21], have: 0, size: 0, width: 0,},
                  attr: 0,
                  ...
                  us: 0,};
    let mut xx: u_int = 0;
    if (*ci).used == 0 as libc::c_int as libc::c_uint { return }
    (*ci).x = (*s).cx;
    (*ci).entry.tqe_next = 0 as *mut screen_write_collect_item;
    ...
    *(*cl).items.tqh_last = ci;
    ...
    log_debug(b"%s: %u %.*s (at %u,%u)\x00" as *const u8 as
              ...
              (*ci).used, (*ci).used as libc::c_int,
              (*cl).data.offset((*ci).x as isize), (*s).cx, (*s).cy);
    if (*s).cx != 0 as libc::c_int as libc::c_uint {
        xx = (*s).cx;
    ...
    grid_view_set_cells((*s).grid, (*s).cx, (*s).cy, &mut (*ci).gc,
...
pub unsafe extern "C" fn screen_write_collect_add(mut ctx:
    ...
    if (*gc).data.width as libc::c_int != 1 as libc::c_int ||
        ...
        collect = 0 as libc::c_int
    } else if !(*s).sel.is_null() { collect = 0 as libc::c_int }
    if collect == 0 {
        screen_write_collect_end(ctx);
        screen_write_collect_flush(ctx, 0 as libc::c_int,
                                   (*::std::mem::transmute::<&[u8; 25],
    ...
    if (*s).cx > sx.wrapping_sub(1 as libc::c_int as libc::c_uint) ||
           (*(*ctx).item).used >
               sx.wrapping_sub(1 as libc::c_int as
    ...
    if (*s).cx > sx.wrapping_sub(1 as libc::c_int as libc::c_uint) {
        ...
        screen_write_linefeed(ctx, 1 as libc::c_int,
                              8 as libc::c_int as u_int);
        screen_write_set_cursor(ctx, 0 as libc::c_int, -(1 as libc::c_int));
    }
    ...
    if (*(*(*ctx).s).write_list.offset((*s).cy as isize)).data.is_null() {
        let ref mut fresh12 =
            (*(*(*ctx).s).write_list.offset((*s).cy as isize)).data;
        *fresh12 =
            xmalloc((*(*(*ctx).s).grid).sx as size_t) as *mut libc::c_char
    }
    let fresh13 = (*ci).used;
    (*ci).used = (*ci).used.wrapping_add(1);
    *(*(*(*ctx).s).write_list.offset((*s).cy as
                                         isize)).data.offset((*s).cx.wrapping_add(fresh13)
                                                                 as isize) =
...
pub unsafe extern "C" fn screen_write_cell(mut ctx: *mut screen_write_ctx,
    ...
    if width == 0 as libc::c_int as libc::c_uint {
        ...
        if !gc.is_null() {
            cx = (*s).cx;
            cy = (*s).cy;
            screen_write_set_cursor(ctx, xx as libc::c_int,
                                    (*s).cy as libc::c_int);
            screen_write_initctx(ctx, &mut ttyctx, 0 as libc::c_int);
            ttyctx.cell = gc;
            tty_write(Some(tty_cmd_cell as
                               unsafe extern "C" fn(_: *mut tty,
                                                    _: *const tty_ctx) -> ()),
                      &mut ttyctx);
            (*s).cx = cx;
    ...
    while xx < (*s).cx.wrapping_add(width) {
        log_debug(b"%s: new padding at %u,%u\x00" as *const u8 as
                      *const libc::c_char,
                  (*::std::mem::transmute::<&[u8; 18],
                                            &[libc::c_char; 18]>(b"screen_write_cell\x00")).as_ptr(),
                  xx, (*s).cy);
    ...
    if skip != 0 {
        if (*s).cx >= (*gl).cellsize {
            ...
            gce =
                &mut *(*gl).celldata.offset((*s).cx as isize) as
                    *mut grid_cell_entry;
            if (*gce).flags as libc::c_int & 0x8 as libc::c_int != 0 {
            ...
            } else if (*gc).bg != (*gce).c2rust_unnamed.data.bg as libc::c_int
             {
                ...
                skip = 0 as libc::c_int
                          ...
                          (*gc).data.data[0 as libc::c_int as usize] as
                              libc::c_int {
                skip = 0 as libc::c_int
            }
        }
    ...
    if selected != 0 &&
           !((*gc).flags as libc::c_int) & 0x10 as libc::c_int != 0 {
        memcpy(&mut tmp_gc as *mut grid_cell as *mut libc::c_void,
               gc as *const libc::c_void,
               ::std::mem::size_of::<grid_cell>() as libc::c_ulong);
        tmp_gc.flags =
            (tmp_gc.flags as libc::c_int | 0x10 as libc::c_int) as u_char;
        ...
        tmp_gc.flags =
            (tmp_gc.flags as libc::c_int & !(0x10 as libc::c_int)) as u_char;
        grid_view_set_cell(gd, (*s).cx, (*s).cy, &mut tmp_gc);
    } else if skip == 0 { grid_view_set_cell(gd, (*s).cx, (*s).cy, gc); }
    if selected != 0 { skip = 0 as libc::c_int }
    /*
    ...
    if (*s).cx <= sx.wrapping_sub(last).wrapping_sub(width) {
        ...
        screen_write_set_cursor(ctx, sx.wrapping_sub(last) as libc::c_int,
                                -(1 as libc::c_int));
    }
...
unsafe extern "C" fn screen_write_combine(mut ctx: *mut screen_write_ctx,
 ...
 -> *const grid_cell {
    ...
    if n > (*s).cx { return 0 as *const grid_cell }
    *xx = (*s).cx.wrapping_sub(n);
    ...
    log_debug(b"%s: %.*s onto %.*s at %u,%u\x00" as *const u8 as
              ...
              gc.data.size as libc::c_int, gc.data.data.as_mut_ptr(), *xx,
              (*s).cy);
    /* Append the data. */
    memcpy(gc.data.data.as_mut_ptr().offset(gc.data.size as libc::c_int as
                                                isize) as *mut libc::c_void,
           (*ud).data.as_ptr() as *const libc::c_void,
           (*ud).size as libc::c_ulong);
    gc.data.size =
        (gc.data.size as libc::c_int + (*ud).size as libc::c_int) as u_char;
    /* Set the new cell. */
...
unsafe extern "C" fn screen_write_overwrite(mut ctx: *mut screen_write_ctx,
    ...
    if width != 1 as libc::c_int as libc::c_uint ||
        ...
        xx =
            (*s).cx.wrapping_add(width).wrapping_sub(1 as libc::c_int as
                                                         libc::c_uint);
        loop  {
            xx = xx.wrapping_add(1);
...
pub unsafe extern "C" fn screen_write_setselection(mut ctx:
    ...
    ttyctx.num = len;
    tty_write(Some(tty_cmd_setselection as
                       unsafe extern "C" fn(_: *mut tty, _: *const tty_ctx)
                           -> ()), &mut ttyctx);
}
...
pub unsafe extern "C" fn screen_write_rawstring(mut ctx:
    ...
    ttyctx.ptr = str as *mut libc::c_void;
    ttyctx.num = len;
    tty_write(Some(tty_cmd_rawstring as
                       unsafe extern "C" fn(_: *mut tty, _: *const tty_ctx)
                           -> ()), &mut ttyctx);
}
/* Turn alternate screen on. */
#[no_mangle]
pub unsafe extern "C" fn screen_write_alternateon(mut ctx:
                                                      *mut screen_write_ctx,
                                                  mut gc: *mut grid_cell,
...
#[no_mangle]
pub unsafe extern "C" fn screen_write_alternateoff(mut ctx:
                                                       *mut screen_write_ctx,
                                                   mut gc: *mut grid_cell,
                                                   mut cursor: libc::c_int) {
    let mut ttyctx: tty_ctx =
