...
extern "C" {
    ...
    #[no_mangle]
    fn layout_parse(_: *mut window, _: *const libc::c_char) -> libc::c_int;
    #[no_mangle]
    fn layout_set_lookup(_: *const libc::c_char) -> libc::c_int;
    #[no_mangle]
    ...
    unsafe {
        {
            let mut init =
                cmd_entry{name:
                              b"select-layout\x00" as *const u8 as
                                  *const libc::c_char,
                          alias:
                              b"selectl\x00" as *const u8 as
                                  *const libc::c_char,
                          args:
                              {
                                  let mut init =
                          ...
                          usage:
                              b"[-Enop] [-t target-pane] [layout-name]\x00" as
                          ...
                          exec:
                              Some(cmd_select_layout_exec as
    ...
    unsafe {
        {
            let mut init =
                cmd_entry{name:
                              b"next-layout\x00" as *const u8 as
                                  *const libc::c_char,
                          alias:
                              b"nextl\x00" as *const u8 as
                                  *const libc::c_char,
                          args:
                              {
                                  let mut init =
                          ...
                          usage:
                              b"[-t target-window]\x00" as *const u8 as
                                  *const libc::c_char,
                          source:
                              cmd_entry_flag{flag: 0,
                                             type_0: CMD_FIND_PANE,
    ...
    unsafe {
        {
            let mut init =
                cmd_entry{name:
                              b"previous-layout\x00" as *const u8 as
                                  *const libc::c_char,
                          alias:
                              b"prevl\x00" as *const u8 as
                                  *const libc::c_char,
                          args:
                              {
                                  let mut init =
 ...
 */
unsafe extern "C" fn cmd_select_layout_exec(mut self_0: *mut cmd,
 ...
 -> cmd_retval {
    ...
    let mut layoutname: *const libc::c_char = 0 as *const libc::c_char;
    let mut oldlayout: *mut libc::c_char = 0 as *mut libc::c_char;
    let mut next: libc::c_int = 0;
    let mut previous: libc::c_int = 0;
    let mut layout: libc::c_int = 0;
    server_unzoom_window(w);
    next =
        (cmd_get_entry(self_0) == &cmd_next_layout_entry as *const cmd_entry)
            as libc::c_int;
    if args_has(args, 'n' as i32 as u_char) != 0 { next = 1 as libc::c_int }
    previous =
        (cmd_get_entry(self_0) ==
             &cmd_previous_layout_entry as *const cmd_entry) as libc::c_int;
    ...
    if next != 0 || previous != 0 {
        if next != 0 { layout_set_next(w); } else { layout_set_previous(w); }
        ...
        match current_block {
            ...
            _ => {
                ...
                } else { layoutname = 0 as *const libc::c_char }
                if !layoutname.is_null() {
                    if layout_parse(w, layoutname) == -(1 as libc::c_int) {
                        cmdq_error(item,
                                   b"can\'t set layout: %s\x00" as *const u8
                                       as *const libc::c_char, layoutname);
                        free((*w).old_layout as *mut libc::c_void);
                        (*w).old_layout = oldlayout;
                        return CMD_RETURN_ERROR
                    }
                } else {
                    free(oldlayout as *mut libc::c_void);
