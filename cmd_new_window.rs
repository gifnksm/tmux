    ...
    unsafe {
        {
            let mut init =
                cmd_entry{name:
                              b"new-window\x00" as *const u8 as
                                  *const libc::c_char,
                          alias:
                              b"neww\x00" as *const u8 as *const libc::c_char,
                          args:
                              {
                                  let mut init =
                                      C2RustUnnamed_32{template:
                                                           b"abc:de:F:kn:Pt:\x00"
                                                               as *const u8 as
                                                               *const libc::c_char,
                                                       lower:
                                                           0 as libc::c_int,
                          ...
                          usage:
                              b"[-abdkP] [-c start-directory] [-e environment] [-F format] [-n window-name] [-t target-window] [command]\x00"
...
unsafe extern "C" fn cmd_new_window_exec(mut self_0: *mut cmd,
 ...
 -> cmd_retval {
    ...
    let mut new_wl: *mut winlink = 0 as *mut winlink;
    ...
    if args_has(args, 'a' as i32 as u_char) != 0 || before != 0 {
        ...
        if idx == -(1 as libc::c_int) { idx = (*target).idx }
    }
    ...
    if args_has(args, 'P' as i32 as u_char) != 0 {
        ...
        cp =
            format_single(item, template, tc, s, new_wl,
