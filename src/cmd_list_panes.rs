...
pub static mut cmd_list_panes_entry: cmd_entry = {
    {
        let mut init = cmd_entry {
            name: b"list-panes\x00" as *const u8 as *const libc::c_char,
            alias: b"lsp\x00" as *const u8 as *const libc::c_char,
            args: {
                let mut init = C2RustUnnamed_32 {
                    template: b"asF:f:t:\x00" as *const u8 as *const libc::c_char,
                    lower: 0i32,
                    upper: 0i32,
                };
                init
            },
            usage: b"[-as] [-F format] [-f filter] [-t target-window]\x00" as *const u8
...
unsafe extern "C" fn cmd_list_panes_exec(
    ...
    } else if args_has(args, 's' as u_char) != 0 {
        cmd_list_panes_session(self_0, s, item, 1i32);
    } else {
        cmd_list_panes_window(self_0, s, wl, item, 0i32);
    }
    return CMD_RETURN_NORMAL;
}
unsafe extern "C" fn cmd_list_panes_server(
    mut self_0: *mut crate::cmd::cmd,
    mut item: *mut crate::cmd_queue::cmdq_item,
) {
    ...
    while !s.is_null() {
        cmd_list_panes_session(self_0, s, item, 2i32);
