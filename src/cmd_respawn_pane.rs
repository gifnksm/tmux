...
pub static mut cmd_respawn_pane_entry: cmd_entry = {
    {
        let mut init = cmd_entry {
            ...
            alias: b"respawnp\x00" as *const u8 as *const libc::c_char,
            args: {
            ...
            usage: b"[-k] [-c start-directory] [-e environment] [-t target-pane] [command]\x00"
...
unsafe extern "C" fn cmd_respawn_pane_exec(
    ...
    if spawn_pane(&mut sc, &mut cause).is_null() {
        cmdq_error(
            item,
            b"respawn pane failed: %s\x00" as *const u8 as *const libc::c_char,
            cause,
        );
