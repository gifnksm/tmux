...
pub static mut cmd_bind_key_entry: cmd_entry = {
    {
        let mut init = cmd_entry {
            name: b"bind-key\x00" as *const u8 as *const libc::c_char,
            alias: b"bind\x00" as *const u8 as *const libc::c_char,
            args: {
            ...
            usage: b"[-nr] [-T key-table] [-N note] key [command [arguments]]\x00" as *const u8
    ...
    if argc != 1i32 {
        ...
        match (*pr).status {
            0 => {
                cmdq_error(
                    item,
                    b"empty command\x00" as *const u8 as *const libc::c_char,
                );
                return CMD_RETURN_ERROR;
            }
            1 => {
