        ...
        && (*c).tty.mouse_drag_flag != 0
    {
        ...
        match type_0 as libc::c_uint {
            1 => {
                ...
                if where_0 as libc::c_uint == STATUS_LEFT as libc::c_int as libc::c_uint {
                    key = KEYC_MOUSEMOVE_STATUS_LEFT as libc::c_ulong as key_code
                }
                if where_0 as libc::c_uint == STATUS_RIGHT as libc::c_int as libc::c_uint {
                    key = KEYC_MOUSEMOVE_STATUS_RIGHT as libc::c_ulong as key_code
                }
                if where_0 as libc::c_uint == STATUS_DEFAULT as libc::c_int as libc::c_uint {
                    key = KEYC_MOUSEMOVE_STATUS_DEFAULT as libc::c_ulong as key_code
                }
                if where_0 as libc::c_uint == BORDER as libc::c_int as libc::c_uint {
...
unsafe extern "C" fn server_client_key_callback(
                    ...
                    loop {
                            ...
                            ) != 0 as libc::c_int
                        {
                            ...
                            loop {
                                ...
                                } else if key0 != KEYC_ANY as libc::c_ulong as libc::c_ulonglong {
                                    key0 = KEYC_ANY as libc::c_ulong as key_code
                                } else {
                                    /*
    ...
    if !s.is_null() && key != KEYC_FOCUS_OUT as libc::c_ulong as libc::c_ulonglong {
    ...
    if cmd_unpack_argv(buf, len, argc, &mut argv) != 0 as libc::c_int {
        ...
        match (*pr).status as libc::c_uint {
            ...
            2 | _ => {
                ...
                cmdq_append(c, cmdq_get_command((*pr).cmdlist, 0 as *mut cmdq_state));
                cmdq_append(
                    c,
                    cmdq_get_callback1(
