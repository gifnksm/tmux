...
extern "C" {
    ...
    #[no_mangle]
    fn tty_putcode_ptr2(_: *mut tty, _: TtyCode, _: *const libc::c_void, _: *const libc::c_void);
    #[no_mangle]
...
unsafe extern "C" fn cmd_refresh_client_exec(
    ...
    if args_has(args, 'A' as u_char) != 0 {
        ...
        if !(!(*tc).flags & 0x2000u64 != 0) {
                ...
                ) != 2i32
            {
                cmdq_error(
                    item,
                    b"bad size argument\x00" as *const u8 as *const libc::c_char,
                );
                return CMD_RETURN_ERROR;
            }
    ...
    cmdq_error(
        item,
        b"not a control client\x00" as *const u8 as *const libc::c_char,
    );
    return CMD_RETURN_ERROR;
}
