...
extern "C" {
    ...
    #[no_mangle]
    fn spawn_pane(_: *mut spawn_context, _: *mut *mut libc::c_char) -> *mut window_pane;
}
...
unsafe extern "C" fn cmd_split_window_exec(
    ...
    p = args_get(args, 'l' as u_char);
    if !p.is_null() {
        plen = strlen(p);
        if *p.offset(plen.wrapping_sub(1u64) as isize) as libc::c_int == '%' as i32 {
            copy = xstrdup(p);
            *copy.offset(plen.wrapping_sub(1u64) as isize) = '\u{0}' as libc::c_char;
            percentage = strtonum(copy, 0i64, 2147483647i64, &mut errstr) as libc::c_int;
            free(copy as *mut libc::c_void);
            if !errstr.is_null() {
                cmdq_error(
                    item,
                    b"percentage %s\x00" as *const u8 as *const libc::c_char,
                    errstr,
                );
            ...
            size =
                args_strtonum(args, 'l' as u_char, 0i64, 2147483647i64, &mut cause) as libc::c_int;
            if !cause.is_null() {
                cmdq_error(
                    item,
                    b"lines %s\x00" as *const u8 as *const libc::c_char,
                    cause,
                );
        ...
        percentage =
            args_strtonum(args, 'p' as u_char, 0i64, 2147483647i64, &mut cause) as libc::c_int;
        if !cause.is_null() {
            cmdq_error(
                item,
                b"create pane failed: -p %s\x00" as *const u8 as *const libc::c_char,
                cause,
            );
            free(cause as *mut libc::c_void);
            return CMD_RETURN_ERROR;
    ...
    input = (args_has(args, 'I' as u_char) != 0 && (*args).argc == 0i32) as libc::c_int;
    flags = 0i32;
    ...
    if input != 0
        || (*args).argc == 1i32 && **(*args).argv.offset(0isize) as libc::c_int == '\u{0}' as i32
    {
        flags |= 0x40i32
    }
    ...
    if lc.is_null() {
        cmdq_error(
            item,
            b"no space for new pane\x00" as *const u8 as *const libc::c_char,
        );
        return CMD_RETURN_ERROR;
    }
    memset(
    ...
    if new_wp.is_null() {
        cmdq_error(
            item,
            b"create pane failed: %s\x00" as *const u8 as *const libc::c_char,
            cause,
        );
        free(cause as *mut libc::c_void);
        return CMD_RETURN_ERROR;
    }
    if input != 0 && window_pane_start_input(new_wp, item, &mut cause) != 0i32 {
    ...
    if args_has(args, 'd' as u_char) == 0 {
        cmd_find_from_winlink_pane(current, wl, new_wp, 0i32);
    }
    ...
    cmd_find_from_winlink_pane(&mut fs, wl, new_wp, 0i32);
    cmdq_insert_hook(
        s,
        item,
        &mut fs as *mut cmd_find_state,
        b"after-split-window\x00" as *const u8 as *const libc::c_char,
    );
