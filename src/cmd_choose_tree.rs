...
pub static mut cmd_choose_client_entry: cmd_entry = {
    {
        let mut init = cmd_entry {
            ...
            usage: b"[-NrZ] [-F format] [-f filter] [-O sort-order] [-t target-pane] [template]\x00"
                as *const u8 as *const libc::c_char,
            source: cmd_entry_flag {
                flag: 0,
                type_0: CMD_FIND_PANE,
...
pub static mut cmd_customize_mode_entry: cmd_entry = {
    {
        let mut init = cmd_entry {
            ...
            usage: b"[-NZ] [-F format] [-f filter] [-t target-pane]\x00" as *const u8
...
unsafe extern "C" fn cmd_choose_tree_exec(
    ...
    } else if cmd_get_entry(self_0) == &cmd_customize_mode_entry as *const cmd_entry {
