...
extern "C" {
    ...
    #[no_mangle]
    fn environ_clear(_: *mut crate::environ::environ, _: *const libc::c_char);
    #[no_mangle]
    fn environ_unset(_: *mut crate::environ::environ, _: *const libc::c_char);
    #[no_mangle]
...
pub static mut cmd_set_environment_entry: cmd_entry = {
    {
        let mut init = cmd_entry {
            name: b"set-environment\x00" as *const u8 as *const libc::c_char,
            ...
            usage: b"[-Fhgru] [-t target-session] name [value]\x00" as *const u8
...
unsafe extern "C" fn cmd_set_environment_exec(
    ...
    if *name as libc::c_int == '\u{0}' as i32 {
        cmdq_error(
            item,
            b"empty variable name\x00" as *const u8 as *const libc::c_char,
        );
    ...
    if !strchr(name, '=' as i32).is_null() {
        cmdq_error(
            item,
            b"variable name contains =\x00" as *const u8 as *const libc::c_char,
        );
        return CMD_RETURN_ERROR;
    }
    if (*args).argc < 2i32 {
    ...
    if args_has(args, 'g' as u_char) != 0 {
        ...
        current_block = 5795734919426202444;
    } else {
        ...
        10043043949733653460 => {
            if args_has(args, 'u' as u_char) != 0 {
                if !value.is_null() {
                    cmdq_error(
                        item,
                        b"can\'t specify a value with -u\x00" as *const u8 as *const libc::c_char,
                    );
                    retval = CMD_RETURN_ERROR
                } else {
                ...
                if !value.is_null() {
                    cmdq_error(
                        item,
                        b"can\'t specify a value with -r\x00" as *const u8 as *const libc::c_char,
                    );
                    retval = CMD_RETURN_ERROR
                } else {
