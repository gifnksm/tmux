...
extern "C" {
    ...
    #[no_mangle]
    fn options_set_parent(_: *mut crate::options::options, _: *mut crate::options::options);
    #[no_mangle]
...
pub static mut cmd_join_pane_entry: cmd_entry = {
    {
        let mut init = cmd_entry {
            ...
            alias: b"joinp\x00" as *const u8 as *const libc::c_char,
            args: {
            ...
            usage: b"[-bdfhv] [-l size] [-s src-pane] [-t dst-pane]\x00" as *const u8
...
pub static mut cmd_move_pane_entry: cmd_entry = {
    {
        let mut init = cmd_entry {
            ...
            alias: b"movep\x00" as *const u8 as *const libc::c_char,
            args: {
                let mut init = C2RustUnnamed_32 {
                    template: b"bdfhvp:l:s:t:\x00" as *const u8 as *const libc::c_char,
                    lower: 0i32,
...
unsafe extern "C" fn cmd_join_pane_exec(
    ...
    if src_wp == dst_wp {
        cmdq_error(
            item,
            b"source and target panes must be different\x00" as *const u8 as *const libc::c_char,
        );
    ...
    if !cause.is_null() {
        cmdq_error(
            item,
            b"size %s\x00" as *const u8 as *const libc::c_char,
            cause,
        );
    ...
    if lc.is_null() {
        cmdq_error(
            item,
            b"create pane failed: pane too small\x00" as *const u8 as *const libc::c_char,
        );
        return CMD_RETURN_ERROR;
    }
    ...
    *(*src_wp).entry.tqe_prev = (*src_wp).entry.tqe_next;
    ...
    if !(*src_wp).entry.tqe_next.is_null() {
        ...
        (*dst_w).panes.tqh_last = &mut (*src_wp).entry.tqe_next
    }
