...
unsafe extern "C" fn cmd_display_message_each(
    ...
    let mut item: *mut crate::cmd_queue::cmdq_item = arg as *mut crate::cmd_queue::cmdq_item;
    cmdq_print(
        item,
...
unsafe extern "C" fn cmd_display_message_exec(
    ...
    if template.is_null() {
        template =
            b"[#{session_name}] #{window_index}:#{window_name}, current pane #{pane_index} - (%H:%M %d-%b-%y)\x00"
                as *const u8 as *const libc::c_char
    }
    /*
