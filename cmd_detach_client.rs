...
extern "C" {
    ...
    #[no_mangle]
    fn server_client_exec(_: *mut client, _: *const libc::c_char);
}
    ...
    unsafe {
        {
            let mut init =
                cmd_entry{name:
                              b"detach-client\x00" as *const u8 as
                                  *const libc::c_char,
                          alias:
                              b"detach\x00" as *const u8 as
                                  *const libc::c_char,
                          args:
                              {
                                  let mut init =
                          ...
                          usage:
                              b"[-aP] [-E shell-command] [-s target-session] [-t target-client]\x00"
                          ...
                          exec:
                              Some(cmd_detach_client_exec as
    ...
    unsafe {
        {
            let mut init =
                cmd_entry{name:
                              b"suspend-client\x00" as *const u8 as
                                  *const libc::c_char,
                          alias:
                              b"suspendc\x00" as *const u8 as
                                  *const libc::c_char,
                          args:
                              {
                                  let mut init =
                          ...
                          usage:
                              b"[-t target-client]\x00" as *const u8 as
                                  *const libc::c_char,
                          source:
                              cmd_entry_flag{flag: 0,
                                             type_0: CMD_FIND_PANE,
 ...
 * Detach a client.
 */
unsafe extern "C" fn cmd_detach_client_exec(mut self_0: *mut cmd,
 ...
 -> cmd_retval {
    ...
    if args_has(args, 's' as i32 as u_char) != 0 {
        s = (*source).s;
        if s.is_null() { return CMD_RETURN_NORMAL }
        loop_0 = clients.tqh_first;
        while !loop_0.is_null() {
            if (*loop_0).session == s {
                if !cmd.is_null() {
                    server_client_exec(loop_0, cmd);
                } else { server_client_detach(loop_0, msgtype); }
            }
    ...
    } else { server_client_detach(tc, msgtype); }
    return CMD_RETURN_STOP;
}
