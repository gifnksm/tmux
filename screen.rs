...
unsafe extern "C" fn screen_free_titles(mut s: *mut screen) {
    let mut title_entry: *mut screen_title_entry =
        0 as *mut screen_title_entry;
    if (*s).titles.is_null() { return }
    loop  {
        title_entry = (*(*s).titles).tqh_first;
        if title_entry.is_null() { break ; }
        if !(*title_entry).entry.tqe_next.is_null() {
            (*(*title_entry).entry.tqe_next).entry.tqe_prev =
                (*title_entry).entry.tqe_prev
        } else { (*(*s).titles).tqh_last = (*title_entry).entry.tqe_prev }
...
/* Create a new screen. */
#[no_mangle]
pub unsafe extern "C" fn screen_init(mut s: *mut screen, mut sx: u_int,
...
pub unsafe extern "C" fn screen_free(mut s: *mut screen) {
    ...
    if !(*s).write_list.is_null() { screen_write_free_list(s); }
    if !(*s).saved_grid.is_null() { grid_destroy((*s).saved_grid); }
    grid_destroy((*s).grid);
    screen_free_titles(s);
}
...
pub unsafe extern "C" fn screen_reset_tabs(mut s: *mut screen) {
    ...
    free((*s).tabs as *mut libc::c_void);
    (*s).tabs =
        calloc(((*(*s).grid).sx.wrapping_add(7 as libc::c_int as libc::c_uint)
    ...
    while i < (*(*s).grid).sx {
        let ref mut fresh0 =
            *(*s).tabs.offset((i >> 3 as libc::c_int) as isize);
        *fresh0 =
            (*fresh0 as libc::c_int |
                 (1 as libc::c_int) <<
                     (i & 0x7 as libc::c_int as libc::c_uint)) as bitstr_t;
        i =
            (i as libc::c_uint).wrapping_add(8 as libc::c_int as libc::c_uint)
                as u_int as u_int
    };
}
/* Set screen cursor style. */
#[no_mangle]
pub unsafe extern "C" fn screen_set_cursor_style(mut s: *mut screen,
                                                 mut style: u_int) {
    if style <= 6 as libc::c_int as libc::c_uint { (*s).cstyle = style };
}
/* Set screen cursor colour. */
#[no_mangle]
pub unsafe extern "C" fn screen_set_cursor_colour(mut s: *mut screen,
                                                  mut colour:
                                                      *const libc::c_char) {
    free((*s).ccolour as *mut libc::c_void);
...
/* Set screen title. */
#[no_mangle]
pub unsafe extern "C" fn screen_set_title(mut s: *mut screen,
                                          mut title: *const libc::c_char)
 -> libc::c_int {
    if utf8_isvalid(title) == 0 { return 0 as libc::c_int }
    free((*s).title as *mut libc::c_void);
...
/* Set screen path. */
#[no_mangle]
pub unsafe extern "C" fn screen_set_path(mut s: *mut screen,
                                         mut path: *const libc::c_char) {
    free((*s).path as *mut libc::c_void);
    utf8_stravis(&mut (*s).path, path,
...
pub unsafe extern "C" fn screen_push_title(mut s: *mut screen) {
    ...
    if (*s).titles.is_null() {
        (*s).titles =
            xmalloc(::std::mem::size_of::<screen_titles>() as libc::c_ulong)
                as *mut screen_titles;
        (*(*s).titles).tqh_first = 0 as *mut screen_title_entry;
        (*(*s).titles).tqh_last = &mut (*(*s).titles).tqh_first
    }
    title_entry =
        xmalloc(::std::mem::size_of::<screen_title_entry>() as libc::c_ulong)
            as *mut screen_title_entry;
    (*title_entry).text = xstrdup((*s).title);
    (*title_entry).entry.tqe_next = (*(*s).titles).tqh_first;
    if !(*title_entry).entry.tqe_next.is_null() {
        (*(*(*s).titles).tqh_first).entry.tqe_prev =
            &mut (*title_entry).entry.tqe_next
    } else { (*(*s).titles).tqh_last = &mut (*title_entry).entry.tqe_next }
...
#[no_mangle]
pub unsafe extern "C" fn screen_resize_cursor(mut s: *mut screen,
    ...
    if !(*s).write_list.is_null() { screen_write_free_list(s); }
    log_debug(b"%s: new size %ux%u, now %ux%u (cursor %u,%u = %u,%u)\x00" as
                  *const u8 as *const libc::c_char,
              (*::std::mem::transmute::<&[u8; 21],
                                        &[libc::c_char; 21]>(b"screen_resize_cursor\x00")).as_ptr(),
              sx, sy, (*(*s).grid).sx, (*(*s).grid).sy, (*s).cx, (*s).cy, cx,
              cy);
    ...
    if sy != (*(*s).grid).sy { screen_resize_y(s, sy, eat_empty, &mut cy); }
    if reflow != 0 { screen_reflow(s, sx, &mut cx, &mut cy, cursor); }
    if cy >= (*(*s).grid).hsize {
        (*s).cx = cx;
    ...
    log_debug(b"%s: cursor finished at %u,%u = %u,%u\x00" as *const u8 as
                  *const libc::c_char,
              (*::std::mem::transmute::<&[u8; 21],
                                        &[libc::c_char; 21]>(b"screen_resize_cursor\x00")).as_ptr(),
              (*s).cx, (*s).cy, cx, cy);
    if !(*s).write_list.is_null() { screen_write_make_list(s); };
}
/* Resize screen. */
#[no_mangle]
pub unsafe extern "C" fn screen_resize(mut s: *mut screen, mut sx: u_int,
                                       mut sy: u_int,
                                       mut reflow: libc::c_int) {
    screen_resize_cursor(s, sx, sy, reflow, 1 as libc::c_int,
                         1 as libc::c_int);
}
unsafe extern "C" fn screen_resize_y(mut s: *mut screen, mut sy: u_int,
    ...
    if sy < oldy {
        ...
        if eat_empty != 0 {
            available =
                oldy.wrapping_sub(1 as libc::c_int as
                                      libc::c_uint).wrapping_sub((*s).cy);
            if available > 0 as libc::c_int as libc::c_uint {
                if available > needed { available = needed }
                grid_view_delete_lines(gd, oldy.wrapping_sub(available),
                                       available, 8 as libc::c_int as u_int);
            }
            needed =
                (needed as libc::c_uint).wrapping_sub(available) as u_int as
                    u_int
        }
        /*
        ...
        if (*gd).flags & 0x1 as libc::c_int != 0 {
            (*gd).hscrolled =
                ((*gd).hscrolled as libc::c_uint).wrapping_add(needed) as
                    u_int as u_int;
            (*gd).hsize =
                ((*gd).hsize as libc::c_uint).wrapping_add(needed) as u_int as
                    u_int
        } else if needed > 0 as libc::c_int as libc::c_uint &&
                      available > 0 as libc::c_int as libc::c_uint {
            if available > needed { available = needed }
            grid_view_delete_lines(gd, 0 as libc::c_int as u_int, available,
                                   8 as libc::c_int as u_int);
            *cy =
                (*cy as libc::c_uint).wrapping_sub(available) as u_int as
                    u_int
        }
    }
    /* Resize line array. */
    ...
    if sy > oldy {
        ...
        if (*gd).flags & 0x1 as libc::c_int != 0 &&
            ...
            if available > needed { available = needed }
            (*gd).hscrolled =
                ((*gd).hscrolled as libc::c_uint).wrapping_sub(available) as
                    u_int as u_int;
            (*gd).hsize =
                ((*gd).hsize as libc::c_uint).wrapping_sub(available) as u_int
                    as u_int
...
/* Set selection. */
#[no_mangle]
pub unsafe extern "C" fn screen_set_selection(mut s: *mut screen,
                                              mut sx: u_int, mut sy: u_int,
                                              mut ex: u_int, mut ey: u_int,
                                              mut rectangle: u_int,
    ...
    if (*s).sel.is_null() {
        (*s).sel =
            xcalloc(1 as libc::c_int as size_t,
                    ::std::mem::size_of::<screen_sel>() as libc::c_ulong) as
                *mut screen_sel
    }
    memcpy(&mut (*(*s).sel).cell as *mut grid_cell as *mut libc::c_void,
...
pub unsafe extern "C" fn screen_hide_selection(mut s: *mut screen) {
    if !(*s).sel.is_null() { (*(*s).sel).hidden = 1 as libc::c_int };
}
/* Check if cell in selection. */
#[no_mangle]
pub unsafe extern "C" fn screen_check_selection(mut s: *mut screen,
                                                mut px: u_int, mut py: u_int)
 -> libc::c_int {
    let mut sel: *mut screen_sel = (*s).sel;
    let mut xx: u_int = 0;
    if sel.is_null() || (*sel).hidden != 0 { return 0 as libc::c_int }
    if (*sel).rectangle != 0 {
        if (*sel).sy < (*sel).ey {
            /* start line < end line -- downward selection. */
            if py < (*sel).sy || py > (*sel).ey { return 0 as libc::c_int }
        } else if (*sel).sy > (*sel).ey {
            /* start line > end line -- upward selection. */
            if py > (*sel).sy || py < (*sel).ey { return 0 as libc::c_int }
        } else if py != (*sel).sy { return 0 as libc::c_int }
        ...
        if (*sel).ex < (*sel).sx {
            ...
            if px < (*sel).ex { return 0 as libc::c_int }
            if px > (*sel).sx { return 0 as libc::c_int }
        } else {
        ...
        if py < (*sel).sy || py > (*sel).ey { return 0 as libc::c_int }
        if py == (*sel).sy && px < (*sel).sx { return 0 as libc::c_int }
        if (*sel).modekeys == 0 as libc::c_int {
            xx =
                if (*sel).ex == 0 as libc::c_int as libc::c_uint {
                    0 as libc::c_int as libc::c_uint
                } else {
                    (*sel).ex.wrapping_sub(1 as libc::c_int as libc::c_uint)
                }
        } else { xx = (*sel).ex }
        if py == (*sel).ey && px > xx { return 0 as libc::c_int }
    } else if (*sel).sy > (*sel).ey {
        /* starting line > ending line -- upward selection. */
        if py > (*sel).sy || py < (*sel).ey { return 0 as libc::c_int }
        if py == (*sel).ey && px < (*sel).ex { return 0 as libc::c_int }
        if (*sel).modekeys == 0 as libc::c_int {
            xx = (*sel).sx.wrapping_sub(1 as libc::c_int as libc::c_uint)
        } else { xx = (*sel).sx }
        if py == (*sel).sy &&
               ((*sel).sx == 0 as libc::c_int as libc::c_uint || px > xx) {
            return 0 as libc::c_int
        }
    } else {
        /* starting line == ending line. */
        if py != (*sel).sy { return 0 as libc::c_int }
        if (*sel).ex < (*sel).sx {
            /* cursor (ex) is on the left */
            if (*sel).modekeys == 0 as libc::c_int {
                xx = (*sel).sx.wrapping_sub(1 as libc::c_int as libc::c_uint)
            } else { xx = (*sel).sx }
            if px > xx || px < (*sel).ex { return 0 as libc::c_int }
        } else {
            /* selection start (sx) is on the left */
            if (*sel).modekeys == 0 as libc::c_int {
                xx =
                    if (*sel).ex == 0 as libc::c_int as libc::c_uint {
                        ...
                        (*sel).ex.wrapping_sub(1 as libc::c_int as
                                                   libc::c_uint)
                    }
            } else { xx = (*sel).ex }
            if px < (*sel).sx || px > xx { return 0 as libc::c_int }
        }
    }
    return 1 as libc::c_int;
}
...
#[no_mangle]
pub unsafe extern "C" fn screen_select_cell(mut s: *mut screen,
                                            mut dst: *mut grid_cell,
                                            mut src: *const grid_cell) {
    if (*s).sel.is_null() || (*(*s).sel).hidden != 0 { return }
    memcpy(dst as *mut libc::c_void,
           &mut (*(*s).sel).cell as *mut grid_cell as *const libc::c_void,
           ::std::mem::size_of::<grid_cell>() as libc::c_ulong);
    utf8_copy(&mut (*dst).data, &(*src).data);
    (*dst).attr =
        ((*dst).attr as libc::c_int & !(0x80 as libc::c_int)) as u_short;
    (*dst).attr =
        ((*dst).attr as libc::c_int |
...
unsafe extern "C" fn screen_reflow(mut s: *mut screen, mut new_x: u_int,
                                   mut cx: *mut u_int, mut cy: *mut u_int,
                                   mut cursor: libc::c_int) {
    let mut wx: u_int = 0;
    let mut wy: u_int = 0;
    if cursor != 0 {
        grid_wrap_position((*s).grid, *cx, *cy, &mut wx, &mut wy);
        log_debug(b"%s: cursor %u,%u is %u,%u\x00" as *const u8 as
                      *const libc::c_char,
                  (*::std::mem::transmute::<&[u8; 14],
                                            &[libc::c_char; 14]>(b"screen_reflow\x00")).as_ptr(),
                  *cx, *cy, wx, wy);
    }
    grid_reflow((*s).grid, new_x);
    if cursor != 0 {
        grid_unwrap_position((*s).grid, cx, cy, wx, wy);
        log_debug(b"%s: new cursor is %u,%u\x00" as *const u8 as
                      *const libc::c_char,
                  (*::std::mem::transmute::<&[u8; 14],
                                            &[libc::c_char; 14]>(b"screen_reflow\x00")).as_ptr(),
                  *cx, *cy);
    } else { *cx = 0 as libc::c_int as u_int; *cy = (*(*s).grid).hsize };
}
/*
 ...
 */
#[no_mangle]
pub unsafe extern "C" fn screen_alternate_on(mut s: *mut screen,
                                             mut gc: *mut grid_cell,
                                             mut cursor: libc::c_int) {
    let mut sx: u_int = 0;
    let mut sy: u_int = 0;
    if !(*s).saved_grid.is_null() { return }
    sx = (*(*s).grid).sx;
    sy = (*(*s).grid).sy;
    (*s).saved_grid = grid_create(sx, sy, 0 as libc::c_int as u_int);
    grid_duplicate_lines((*s).saved_grid, 0 as libc::c_int as u_int,
                         (*s).grid, (*(*s).grid).hsize, sy);
    if cursor != 0 { (*s).saved_cx = (*s).cx; (*s).saved_cy = (*s).cy }
    memcpy(&mut (*s).saved_cell as *mut grid_cell as *mut libc::c_void,
           gc as *const libc::c_void,
           ::std::mem::size_of::<grid_cell>() as libc::c_ulong);
    grid_view_clear((*s).grid, 0 as libc::c_int as u_int,
                    0 as libc::c_int as u_int, sx, sy,
                    8 as libc::c_int as u_int);
...
#[no_mangle]
pub unsafe extern "C" fn screen_alternate_off(mut s: *mut screen,
                                              mut gc: *mut grid_cell,
                                              mut cursor: libc::c_int) {
    let mut sx: u_int = 0;
    let mut sy: u_int = 0;
                                                                                  ...
                                                                                  libc::c_uint)
       {
        ...
        if !gc.is_null() {
            memcpy(gc as *mut libc::c_void,
                   &mut (*s).saved_cell as *mut grid_cell as
                       *const libc::c_void,
                   ::std::mem::size_of::<grid_cell>() as libc::c_ulong);
        }
    }
    ...
    grid_duplicate_lines((*s).grid, (*(*s).grid).hsize, (*s).saved_grid,
