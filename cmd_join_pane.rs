    ...
    unsafe {
        {
            let mut init =
                cmd_entry{name:
                              b"join-pane\x00" as *const u8 as
                                  *const libc::c_char,
                          alias:
                              b"joinp\x00" as *const u8 as
                                  *const libc::c_char,
                          args:
                              {
                                  let mut init =
                          ...
                          usage:
                              b"[-bdfhv] [-l size] [-s src-pane] [-t dst-pane]\x00"
    ...
    unsafe {
        {
            let mut init =
                cmd_entry{name:
                              b"move-pane\x00" as *const u8 as
                                  *const libc::c_char,
                          alias:
                              b"movep\x00" as *const u8 as
                                  *const libc::c_char,
                          args:
                              {
                                  let mut init =
 ...
 */
unsafe extern "C" fn cmd_join_pane_exec(mut self_0: *mut cmd,
 ...
 -> cmd_retval {
    ...
    let mut dst_s: *mut session = 0 as *mut session;
    let mut src_wl: *mut winlink = 0 as *mut winlink;
    let mut dst_wl: *mut winlink = 0 as *mut winlink;
    ...
    if src_wp == dst_wp {
        cmdq_error(item,
                   b"source and target panes must be different\x00" as
                       *const u8 as *const libc::c_char);
        return CMD_RETURN_ERROR
    }
    type_0 = LAYOUT_TOPBOTTOM;
    if args_has(args, 'h' as i32 as u_char) != 0 { type_0 = LAYOUT_LEFTRIGHT }
    size = -(1 as libc::c_int);
    if args_has(args, 'l' as i32 as u_char) != 0 {
        if type_0 as libc::c_uint ==
               LAYOUT_TOPBOTTOM as libc::c_int as libc::c_uint {
            size =
                args_percentage(args, 'l' as i32 as u_char,
                                0 as libc::c_int as libc::c_longlong,
                                2147483647 as libc::c_int as libc::c_longlong,
                                (*dst_wp).sy as libc::c_longlong, &mut cause)
                    as libc::c_int
        } else {
            size =
                args_percentage(args, 'l' as i32 as u_char,
                                0 as libc::c_int as libc::c_longlong,
                                2147483647 as libc::c_int as libc::c_longlong,
                                (*dst_wp).sx as libc::c_longlong, &mut cause)
                    as libc::c_int
        }
        ...
        percentage =
            args_strtonum(args, 'p' as i32 as u_char,
                          0 as libc::c_int as libc::c_longlong,
                          100 as libc::c_int as libc::c_longlong, &mut cause)
                as libc::c_int;
        if cause.is_null() {
            if type_0 as libc::c_uint ==
                   LAYOUT_TOPBOTTOM as libc::c_int as libc::c_uint {
                size =
                    (*dst_wp).sy.wrapping_mul(percentage as
                ...
                size =
                    (*dst_wp).sx.wrapping_mul(percentage as
                                                  libc::c_uint).wrapping_div(100
                                                                                 as
                                                                                 libc::c_int
                                                                                 as
    ...
    if !cause.is_null() {
        cmdq_error(item, b"size %s\x00" as *const u8 as *const libc::c_char,
                   cause);
    ...
    if lc.is_null() {
        cmdq_error(item,
                   b"create pane failed: pane too small\x00" as *const u8 as
                       *const libc::c_char);
        return CMD_RETURN_ERROR
    }
    ...
    } else { (*src_w).panes.tqh_last = (*src_wp).entry.tqe_prev }
    *(*src_wp).entry.tqe_prev = (*src_wp).entry.tqe_next;
    ...
    if !(*src_wp).entry.tqe_next.is_null() {
        (*(*src_wp).entry.tqe_next).entry.tqe_prev =
            &mut (*src_wp).entry.tqe_next
    } else { (*dst_w).panes.tqh_last = &mut (*src_wp).entry.tqe_next }
    (*dst_wp).entry.tqe_next = src_wp;
    (*src_wp).entry.tqe_prev = &mut (*dst_wp).entry.tqe_next;
    ...
    } else { server_status_session(dst_s); }
    if window_count_panes(src_w) == 0 as libc::c_int as libc::c_uint {
        ...
        notify_window(b"window-layout-changed\x00" as *const u8 as
                          *const libc::c_char, src_w);
    }
    notify_window(b"window-layout-changed\x00" as *const u8 as
                      *const libc::c_char, dst_w);
