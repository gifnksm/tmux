...
extern "C" {
    ...
    #[no_mangle]
    fn format_trim_right(_: *const libc::c_char, _: u_int)
     -> *mut libc::c_char;
    #[no_mangle]
    fn format_trim_left(_: *const libc::c_char, _: u_int)
     -> *mut libc::c_char;
    #[no_mangle]
    ...
    #[no_mangle]
    fn window_copy_get_line(_: *mut window_pane, _: u_int)
     -> *mut libc::c_char;
    #[no_mangle]
    fn window_copy_get_word(_: *mut window_pane, _: u_int, _: u_int)
     -> *mut libc::c_char;
    #[no_mangle]
    static mut all_window_modes: [*const window_mode; 0];
    #[no_mangle]
    fn window_pane_search(_: *mut window_pane, _: *const libc::c_char,
                          _: libc::c_int, _: libc::c_int) -> u_int;
    #[no_mangle]
    ...
    #[no_mangle]
    fn regsub(_: *const libc::c_char, _: *const libc::c_char,
...
static mut format_jobs: format_job_tree =
    {
        let mut init =
            format_job_tree{rbh_root:
                                0 as *const format_job as *mut format_job,};
        init
    };
...
unsafe extern "C" fn format_job_cmp(mut fj1: *mut format_job,
    ...
    if (*fj1).tag < (*fj2).tag { return -(1 as libc::c_int) }
    if (*fj1).tag > (*fj2).tag { return 1 as libc::c_int }
        ...
        if (*parent).entry.rbe_left == elm {
            ...
            if ((*tmp).entry.rbe_left.is_null() ||
                ...
                if (*tmp).entry.rbe_left.is_null() ||
                       (*(*tmp).entry.rbe_left).entry.rbe_color ==
                           0 as libc::c_int {
                    let mut oright: *mut format_entry =
                        0 as *mut format_entry;
                    oright = (*tmp).entry.rbe_right;
                    if !oright.is_null() {
                        (*oright).entry.rbe_color = 0 as libc::c_int
                    }
...
unsafe extern "C" fn format_logging(mut ft: *mut format_tree) -> libc::c_int {
    return (log_get_level() != 0 as libc::c_int ||
                (*ft).flags & 0x8 as libc::c_int != 0) as libc::c_int;
}
/* Log a message if verbose. */
unsafe extern "C" fn format_log1(mut es: *mut format_expand_state,
    ...
    static mut spaces: [libc::c_char; 11] =
        unsafe {
            *::std::mem::transmute::<&[u8; 11],
                                     &[libc::c_char; 11]>(b"          \x00")
        };
    if format_logging(ft) == 0 { return }
    ap = args.clone();
    ...
    if !(*ft).item.is_null() && (*ft).flags & 0x8 as libc::c_int != 0 {
        cmdq_print((*ft).item,
                   ...
                   (*es).loop_0, spaces.as_ptr(), s);
    }
    free(s as *mut libc::c_void);
}
/* Copy expand state. */
unsafe extern "C" fn format_copy_state(mut to: *mut format_expand_state,
...
unsafe extern "C" fn format_job_update(mut job: *mut job) {
    ...
    if (*fj).status != 0 && (*fj).last != t {
        if !(*fj).client.is_null() { server_status_client((*fj).client); }
        (*fj).last = t
    };
}
 ...
 */
/* Find a job. */
unsafe extern "C" fn format_job_get(mut es: *mut format_expand_state,
 ...
 -> *mut libc::c_char {
    ...
    let mut fj0: format_job =
        format_job{client: 0 as *mut client,
                   ...
                   status: 0,
                   entry:
                       C2RustUnnamed_33{rbe_left: 0 as *mut format_job,
    ...
    let mut next: format_expand_state =
        format_expand_state{ft: 0 as *mut format_tree,
                            loop_0: 0,
                            time: 0,
                            flags: 0,};
    ...
    if (*fj).expanded.is_null() ||
           strcmp(expanded, (*fj).expanded) != 0 as libc::c_int {
        free((*fj).expanded as *mut libc::c_void);
        (*fj).expanded = xstrdup(expanded);
        force = 1 as libc::c_int
    } else { force = (*ft).flags & 0x2 as libc::c_int }
    t = time(0 as *mut time_t);
    if force != 0 && !(*fj).job.is_null() { job_free((*fj).job); }
    if force != 0 || (*fj).job.is_null() && (*fj).last != t {
        (*fj).job =
            job_run(expanded, 0 as *mut session,
                    server_client_get_cwd((*ft).client, 0 as *mut session),
                    Some(format_job_update as
                             unsafe extern "C" fn(_: *mut job) -> ()),
                    Some(format_job_complete as
                             unsafe extern "C" fn(_: *mut job) -> ()), None,
                    fj as *mut libc::c_void, 0x1 as libc::c_int,
...
unsafe extern "C" fn format_job_tidy(mut jobs: *mut format_job_tree,
    ...
    while !fj.is_null() &&
              { fj1 = format_job_tree_RB_NEXT(fj); (1 as libc::c_int) != 0 } {
        if !(force == 0 &&
                 ((*fj).last > now ||
                      now - (*fj).last < 3600 as libc::c_int as libc::c_long))
           {
            ...
            log_debug(b"%s: %s\x00" as *const u8 as *const libc::c_char,
                      ...
                      (*fj).cmd);
            if !(*fj).job.is_null() { job_free((*fj).job); }
            free((*fj).expanded as *mut libc::c_void);
...
pub unsafe extern "C" fn format_lost_client(mut c: *mut client) {
    if !(*c).jobs.is_null() { format_job_tidy((*c).jobs, 1 as libc::c_int); }
...
unsafe extern "C" fn format_job_timer(mut fd: libc::c_int,
    ...
    let mut tv: timeval =
        {
            let mut init =
                timeval{tv_sec: 60 as libc::c_int as __time_t, tv_usec: 0,};
            init
        };
...
unsafe extern "C" fn format_cb_session_attached_list(mut ft: *mut format_tree)
 -> *mut libc::c_char {
...
unsafe extern "C" fn format_cb_window_linked_sessions_list(mut ft:
                                                               *mut format_tree)
 -> *mut libc::c_char {
...
unsafe extern "C" fn format_cb_window_active_sessions(mut ft:
                                                          *mut format_tree)
 -> *mut libc::c_char {
    let mut w: *mut window = (*(*ft).wl).window;
    let mut wl: *mut winlink = 0 as *mut winlink;
    ...
    wl = (*w).winlinks.tqh_first;
    while !wl.is_null() {
        if (*(*wl).session).curw == wl { n = n.wrapping_add(1) }
        wl = (*wl).wentry.tqe_next
    }
    xasprintf(&mut value as *mut *mut libc::c_char,
              b"%u\x00" as *const u8 as *const libc::c_char, n);
    return value;
}
/* Callback for window_active_sessions_list. */
unsafe extern "C" fn format_cb_window_active_sessions_list(mut ft:
                                                               *mut format_tree)
 -> *mut libc::c_char {
    let mut w: *mut window = (*(*ft).wl).window;
    let mut wl: *mut winlink = 0 as *mut winlink;
...
unsafe extern "C" fn format_cb_window_active_clients(mut ft: *mut format_tree)
 -> *mut libc::c_char {
    let mut w: *mut window = (*(*ft).wl).window;
    let mut loop_0: *mut client = 0 as *mut client;
    ...
    while !loop_0.is_null() {
        ...
        if !client_session.is_null() {
            if w == (*(*client_session).curw).window { n = n.wrapping_add(1) }
        }
        loop_0 = (*loop_0).entry.tqe_next
    }
    xasprintf(&mut value as *mut *mut libc::c_char,
...
unsafe extern "C" fn format_cb_window_active_clients_list(mut ft:
                                                              *mut format_tree)
 -> *mut libc::c_char {
    let mut w: *mut window = (*(*ft).wl).window;
    let mut loop_0: *mut client = 0 as *mut client;
...
unsafe extern "C" fn format_cb_window_layout(mut ft: *mut format_tree)
 -> *mut libc::c_char {
    ...
    if w.is_null() { return 0 as *mut libc::c_char }
...
unsafe extern "C" fn format_cb_start_command(mut ft: *mut format_tree)
 -> *mut libc::c_char {
    ...
    if wp.is_null() { return 0 as *mut libc::c_char }
...
unsafe extern "C" fn format_cb_current_command(mut ft: *mut format_tree)
 -> *mut libc::c_char {
    ...
    let mut value: *mut libc::c_char = 0 as *mut libc::c_char;
    if wp.is_null() || (*wp).shell.is_null() { return 0 as *mut libc::c_char }
...
unsafe extern "C" fn format_cb_current_path(mut ft: *mut format_tree)
 -> *mut libc::c_char {
    ...
    if cwd.is_null() { return 0 as *mut libc::c_char }
    return xstrdup(cwd);
}
...
unsafe extern "C" fn format_cb_history_bytes(mut ft: *mut format_tree)
 -> *mut libc::c_char {
    ...
    while i < (*gd).hsize.wrapping_add((*gd).sy) {
        ...
        size =
            (size as
                 libc::c_ulong).wrapping_add(((*gl).cellsize as
        ...
        size =
            (size as
                 libc::c_ulong).wrapping_add(((*gl).extdsize as
                                                  libc::c_ulong).wrapping_mul(::std::mem::size_of::<grid_extd_entry>()
                                                                                  as
                                                                                  libc::c_ulong))
                as size_t as size_t;
        i = i.wrapping_add(1)
    ...
    size =
        (size as
             libc::c_ulong).wrapping_add(((*gd).hsize.wrapping_add((*gd).sy)
                                              as
...
unsafe extern "C" fn format_cb_history_all_bytes(mut ft: *mut format_tree)
 -> *mut libc::c_char {
    let mut wp: *mut window_pane = (*ft).wp;
    let mut gd: *mut grid = 0 as *mut grid;
    let mut gl: *mut grid_line = 0 as *mut grid_line;
    ...
    while i < lines {
        gl = grid_get_line(gd, i);
        cells =
            (cells as libc::c_uint).wrapping_add((*gl).cellsize) as u_int as
                u_int;
        extended_cells =
            (extended_cells as libc::c_uint).wrapping_add((*gl).extdsize) as
                u_int as u_int;
        i = i.wrapping_add(1)
    }
    xasprintf(&mut value as *mut *mut libc::c_char,
...
unsafe extern "C" fn format_cb_pane_tabs(mut ft: *mut format_tree)
 -> *mut libc::c_char {
    ...
    while i < (*(*wp).base.grid).sx {
        if !(*(*wp).base.tabs.offset((i >> 3 as libc::c_int) as isize) as
                 libc::c_int &
                 (1 as libc::c_int) <<
                     (i & 0x7 as libc::c_int as libc::c_uint) == 0) {
            if evbuffer_get_length(buffer) > 0 as libc::c_int as libc::c_ulong
               {
                evbuffer_add(buffer,
                             b",\x00" as *const u8 as *const libc::c_char as
...
unsafe extern "C" fn format_cb_session_group_list(mut ft: *mut format_tree)
 -> *mut libc::c_char {
    ...
    if sg.is_null() { return 0 as *mut libc::c_char }
    buffer = evbuffer_new();
    if buffer.is_null() {
        fatalx(b"out of memory\x00" as *const u8 as *const libc::c_char);
    }
...
unsafe extern "C" fn format_cb_session_group_attached_list(mut ft:
                                                               *mut format_tree)
 -> *mut libc::c_char {
    let mut s: *mut session = (*ft).s;
    let mut client_session: *mut session = 0 as *mut session;
...
unsafe extern "C" fn format_cb_pane_in_mode(mut ft: *mut format_tree)
 -> *mut libc::c_char {
    let mut wp: *mut window_pane = (*ft).wp;
    let mut n: u_int = 0 as libc::c_int as u_int;
...
unsafe extern "C" fn format_cb_pane_at_top(mut ft: *mut format_tree)
 -> *mut libc::c_char {
    let mut wp: *mut window_pane = (*ft).wp;
    ...
    xasprintf(&mut value as *mut *mut libc::c_char,
              b"%d\x00" as *const u8 as *const libc::c_char, flag);
    return value;
}
/* Callback for pane_at_bottom. */
unsafe extern "C" fn format_cb_pane_at_bottom(mut ft: *mut format_tree)
 -> *mut libc::c_char {
    let mut wp: *mut window_pane = (*ft).wp;
    let mut w: *mut window = 0 as *mut window;
    let mut status: libc::c_int = 0;
    ...
    if status == 2 as libc::c_int {
        flag =
            ((*wp).yoff.wrapping_add((*wp).sy) ==
                 (*w).sy.wrapping_sub(1 as libc::c_int as libc::c_uint)) as
                libc::c_int
    } else {
...
unsafe extern "C" fn format_cb_cursor_character(mut ft: *mut format_tree)
 -> *mut libc::c_char {
    let mut wp: *mut window_pane = (*ft).wp;
    ...
    let mut value: *mut libc::c_char = 0 as *mut libc::c_char;
    if wp.is_null() { return 0 as *mut libc::c_char }
    grid_view_get_cell((*wp).base.grid, (*wp).base.cx, (*wp).base.cy,
                       &mut gc);
    if !(gc.flags as libc::c_int) & 0x4 as libc::c_int != 0 {
        xasprintf(&mut value as *mut *mut libc::c_char,
                  b"%.*s\x00" as *const u8 as *const libc::c_char,
                  gc.data.size as libc::c_int, gc.data.data.as_mut_ptr());
    }
    return value;
}
/* Return word at given coordinates. Caller frees. */
#[no_mangle]
pub unsafe extern "C" fn format_grid_word(mut gd: *mut grid, mut x: u_int,
                                          mut y: u_int) -> *mut libc::c_char {
    ...
    loop  {
        ...
        if utf8_cstrhas(ws, &mut gc.data) != 0 {
            ...
            if x == 0 as libc::c_int as libc::c_uint {
                if y == 0 as libc::c_int as libc::c_uint { break ; }
                gl =
                    grid_peek_line(gd,
    ...
    loop  {
        if found != 0 {
            ...
            if end == 0 as libc::c_int as libc::c_uint ||
                ...
                if y ==
                       (*gd).hsize.wrapping_add((*gd).sy).wrapping_sub(1 as
                                                                           libc::c_int
                                                                           as
                                                                           libc::c_uint)
                   {
        ...
        grid_get_cell(gd, x, y, &mut gc);
        if gc.flags as libc::c_int & 0x4 as libc::c_int != 0 { break ; }
        if utf8_cstrhas(ws, &mut gc.data) != 0 { break ; }
        ud =
            xreallocarray(ud as *mut libc::c_void,
                          size.wrapping_add(2 as libc::c_int as
                                                libc::c_ulong),
                          ::std::mem::size_of::<utf8_data>() as libc::c_ulong)
                as *mut utf8_data;
        let fresh0 = size;
        size = size.wrapping_add(1);
        memcpy(&mut *ud.offset(fresh0 as isize) as *mut utf8_data as
                   *mut libc::c_void,
...
unsafe extern "C" fn format_cb_mouse_word(mut ft: *mut format_tree)
 -> *mut libc::c_char {
    ...
    let mut y: u_int = 0;
    let mut s: *mut libc::c_char = 0 as *mut libc::c_char;
    if (*ft).m.valid == 0 { return 0 as *mut libc::c_char }
    wp =
        cmd_mouse_pane(&mut (*ft).m, 0 as *mut *mut session,
                       0 as *mut *mut winlink);
    if wp.is_null() { return 0 as *mut libc::c_char }
    if cmd_mouse_at(wp, &mut (*ft).m, &mut x, &mut y, 0 as libc::c_int) !=
           0 as libc::c_int {
        return 0 as *mut libc::c_char
    }
    if !(*wp).modes.tqh_first.is_null() {
        if (*(*wp).modes.tqh_first).mode ==
               &window_copy_mode as *const window_mode ||
               (*(*wp).modes.tqh_first).mode ==
...
#[no_mangle]
pub unsafe extern "C" fn format_grid_line(mut gd: *mut grid, mut y: u_int)
 -> *mut libc::c_char {
    let mut gc: grid_cell =
        grid_cell{data: utf8_data{data: [0; 21], have: 0, size: 0, width: 0,},
                  attr: 0,
    ...
    while x < grid_line_length(gd, y) {
        ...
        let fresh1 = size;
        size = size.wrapping_add(1);
        memcpy(&mut *ud.offset(fresh1 as isize) as *mut utf8_data as
                   *mut libc::c_void,
               &mut gc.data as *mut utf8_data as *const libc::c_void,
               ::std::mem::size_of::<utf8_data>() as libc::c_ulong);
        x = x.wrapping_add(1)
...
unsafe extern "C" fn format_cb_mouse_line(mut ft: *mut format_tree)
 -> *mut libc::c_char {
    let mut wp: *mut window_pane = 0 as *mut window_pane;
    let mut gd: *mut grid = 0 as *mut grid;
    let mut x: u_int = 0;
...
#[no_mangle]
pub unsafe extern "C" fn format_merge(mut ft: *mut format_tree,
...
/* Get format pane. */
#[no_mangle]
pub unsafe extern "C" fn format_get_pane(mut ft: *mut format_tree)
 -> *mut window_pane {
    return (*ft).wp;
}
/* Add item bits to tree. */
unsafe extern "C" fn format_create_add_item(mut ft: *mut format_tree,
    ...
    if (*m).valid != 0 &&
           {
               wp =
                   cmd_mouse_pane(m, 0 as *mut *mut session,
                                  0 as *mut *mut winlink);
               !wp.is_null()
           } {
        format_add(ft, b"mouse_pane\x00" as *const u8 as *const libc::c_char,
                   b"%%%u\x00" as *const u8 as *const libc::c_char, (*wp).id);
        if cmd_mouse_at(wp, m, &mut x, &mut y, 0 as libc::c_int) ==
               0 as libc::c_int {
            format_add(ft, b"mouse_x\x00" as *const u8 as *const libc::c_char,
                       b"%u\x00" as *const u8 as *const libc::c_char, x);
            format_add(ft, b"mouse_y\x00" as *const u8 as *const libc::c_char,
                       b"%u\x00" as *const u8 as *const libc::c_char, y);
            format_add_cb(ft,
                          b"mouse_word\x00" as *const u8 as
                              *const libc::c_char,
                          Some(format_cb_mouse_word as
                                   unsafe extern "C" fn(_: *mut format_tree)
                                       -> *mut libc::c_char));
            format_add_cb(ft,
                          b"mouse_line\x00" as *const u8 as
                              *const libc::c_char,
                          Some(format_cb_mouse_line as
                                   unsafe extern "C" fn(_: *mut format_tree)
                                       -> *mut libc::c_char));
        }
    }
    memcpy(&mut (*ft).m as *mut mouse_event as *mut libc::c_void,
...
pub unsafe extern "C" fn format_create(mut c: *mut client,
 ...
 -> *mut format_tree {
    ...
    if event_initialized(&mut format_job_event) == 0 {
        event_set(&mut format_job_event, -(1 as libc::c_int),
                  0 as libc::c_int as libc::c_short,
                  Some(format_job_timer as
                           unsafe extern "C" fn(_: libc::c_int,
                                                _: libc::c_short,
                                                _: *mut libc::c_void) -> ()),
                  0 as *mut libc::c_void);
        format_job_timer(-(1 as libc::c_int),
                         0 as libc::c_int as libc::c_short,
                         0 as *mut libc::c_void);
    }
    ft =
    ...
    while !(*wm).is_null() {
        if !(**wm).default_format.is_null() {
            xsnprintf(tmp.as_mut_ptr(),
                      ...
                      (**wm).name);
            tmp[strcspn(tmp.as_mut_ptr(),
                        b"-\x00" as *const u8 as *const libc::c_char) as
                    usize] = '_' as i32 as libc::c_char;
            format_add(ft, tmp.as_mut_ptr(),
                       b"%s\x00" as *const u8 as *const libc::c_char,
                       (**wm).default_format);
        }
        wm = wm.offset(1)
    }
    if !item.is_null() { format_create_add_item(ft, item); }
    return ft;
}
/* Free a tree. */
#[no_mangle]
pub unsafe extern "C" fn format_free(mut ft: *mut format_tree) {
    ...
    while !fe.is_null() &&
              { fe1 = format_entry_tree_RB_NEXT(fe); (1 as libc::c_int) != 0 }
          {
    ...
    if !(*ft).client.is_null() { server_client_unref((*ft).client); }
    free(ft as *mut libc::c_void);
}
/* Walk each format. */
#[no_mangle]
pub unsafe extern "C" fn format_each(mut ft: *mut format_tree,
                                     mut cb:
    ...
    while !fe.is_null() {
        if (*fe).time != 0 as libc::c_int as libc::c_long {
            ...
            cb.expect("non-null function pointer")((*fe).key, s.as_mut_ptr(),
                                                   arg);
        } else {
            if (*fe).value.is_null() && (*fe).cb.is_some() {
                (*fe).value =
                    (*fe).cb.expect("non-null function pointer")(ft);
                if (*fe).value.is_null() {
                    (*fe).value =
                        xstrdup(b"\x00" as *const u8 as *const libc::c_char)
                }
            }
            cb.expect("non-null function pointer")((*fe).key, (*fe).value,
                                                   arg);
        }
        fe = format_entry_tree_RB_NEXT(fe)
    };
...
#[no_mangle]
pub unsafe extern "C" fn format_add(mut ft: *mut format_tree,
...
/* Add a key and time. */
#[no_mangle]
pub unsafe extern "C" fn format_add_tv(mut ft: *mut format_tree,
                                       mut key: *const libc::c_char,
                                       mut tv: *mut timeval) {
    let mut fe: *mut format_entry = 0 as *mut format_entry;
    let mut fe_now: *mut format_entry = 0 as *mut format_entry;
...
#[no_mangle]
pub unsafe extern "C" fn format_add_cb(mut ft: *mut format_tree,
                                       mut key: *const libc::c_char,
                                       mut cb: format_cb) {
    let mut fe: *mut format_entry = 0 as *mut format_entry;
    let mut fe_now: *mut format_entry = 0 as *mut format_entry;
...
unsafe extern "C" fn format_quote(mut s: *const libc::c_char)
 -> *mut libc::c_char {
    let mut cp: *const libc::c_char = 0 as *const libc::c_char;
    let mut out: *mut libc::c_char = 0 as *mut libc::c_char;
    let mut at: *mut libc::c_char = 0 as *mut libc::c_char;
    out =
        xmalloc(strlen(s).wrapping_mul(2 as libc::c_int as
    ...
    while *cp as libc::c_int != '\u{0}' as i32 {
        if !strchr(b"|&;<>()$`\\\"\'*?[# =%\x00" as *const u8 as
                       *const libc::c_char, *cp as libc::c_int).is_null() {
            let fresh2 = at;
            at = at.offset(1);
...
unsafe extern "C" fn format_pretty_time(mut t: time_t) -> *mut libc::c_char {
    ...
    if age < (24 as libc::c_int * 3600 as libc::c_int) as libc::c_long {
        strftime(s.as_mut_ptr(),
                 ::std::mem::size_of::<[libc::c_char; 6]>() as libc::c_ulong,
                 b"%H:%M\x00" as *const u8 as *const libc::c_char, &mut tm);
        return xstrdup(s.as_mut_ptr())
    }
    /* This month or last 28 days. */
    if tm.tm_year == now_tm.tm_year && tm.tm_mon == now_tm.tm_mon ||
           age <
               (28 as libc::c_int * 24 as libc::c_int * 3600 as libc::c_int)
                   as libc::c_long {
        strftime(s.as_mut_ptr(),
                 ::std::mem::size_of::<[libc::c_char; 6]>() as libc::c_ulong,
                 b"%a%d\x00" as *const u8 as *const libc::c_char, &mut tm);
        return xstrdup(s.as_mut_ptr())
    }
    /* Last 12 months. */
    if tm.tm_year == now_tm.tm_year && tm.tm_mon < now_tm.tm_mon ||
           tm.tm_year == now_tm.tm_year - 1 as libc::c_int &&
               tm.tm_mon > now_tm.tm_mon {
        strftime(s.as_mut_ptr(),
                 ::std::mem::size_of::<[libc::c_char; 6]>() as libc::c_ulong,
                 b"%d%b\x00" as *const u8 as *const libc::c_char, &mut tm);
        return xstrdup(s.as_mut_ptr())
    }
    /* Older than that. */
    strftime(s.as_mut_ptr(),
             ::std::mem::size_of::<[libc::c_char; 6]>() as libc::c_ulong,
             b"%h%y\x00" as *const u8 as *const libc::c_char, &mut tm);
    return xstrdup(s.as_mut_ptr());
}
/* Find a format entry. */
 ...
 -> *mut libc::c_char {
    ...
    let mut fe_find: format_entry =
        format_entry{key: 0 as *mut libc::c_char,
                     ...
                     cb: None,
                     entry:
                         C2RustUnnamed_26{rbe_left: 0 as *mut format_entry,
    ...
    if o.is_null() && !(*ft).wp.is_null() {
        o =
            options_parse_get((*(*ft).wp).options, key, &mut idx,
                              0 as libc::c_int)
    }
    if o.is_null() && !(*ft).w.is_null() {
        o =
            options_parse_get((*(*ft).w).options, key, &mut idx,
                              0 as libc::c_int)
    }
    if o.is_null() {
        o =
            options_parse_get(global_w_options, key, &mut idx,
                              0 as libc::c_int)
    }
    if o.is_null() && !(*ft).s.is_null() {
        o =
            options_parse_get((*(*ft).s).options, key, &mut idx,
                              0 as libc::c_int)
    }
    if o.is_null() {
        o =
            options_parse_get(global_s_options, key, &mut idx,
                              0 as libc::c_int)
    }
    if !o.is_null() {
        ...
        if !fe.is_null() {
            ...
            if !modifiers & 0x1 as libc::c_int != 0 {
                ...
                } else { current_block = 8180496224585318153; }
            } else { current_block = 8180496224585318153; }
            match current_block {
                7121915885831525550 => { }
    ...
    if modifiers & 0x1 as libc::c_int != 0 {
        if t == 0 as libc::c_int as libc::c_long && !found.is_null() {
            t =
                strtonum(found, 0 as libc::c_int as libc::c_longlong,
                         9223372036854775807 as libc::c_long as
                             libc::c_longlong, &mut errstr) as time_t;
            if !errstr.is_null() { t = 0 as libc::c_int as time_t }
            ...
            if !time_format.is_null() {
                ...
                strftime(s.as_mut_ptr(),
                         ::std::mem::size_of::<[libc::c_char; 512]>() as
                             libc::c_ulong, time_format, &mut tm);
            } else {
    ...
    if t != 0 as libc::c_int as libc::c_long {
        xasprintf(&mut found as *mut *mut libc::c_char,
                  b"%lld\x00" as *const u8 as *const libc::c_char,
                  t as libc::c_longlong);
    } else if found.is_null() { return 0 as *mut libc::c_char }
...
unsafe extern "C" fn format_strip(mut s: *const libc::c_char)
 -> *mut libc::c_char {
    let mut out: *mut libc::c_char = 0 as *mut libc::c_char;
    let mut cp: *mut libc::c_char = 0 as *mut libc::c_char;
    let mut brackets: libc::c_int = 0 as libc::c_int;
    out =
        xmalloc(strlen(s).wrapping_add(1 as libc::c_int as libc::c_ulong)) as
            *mut libc::c_char;
    cp = out;
    while *s as libc::c_int != '\u{0}' as i32 {
        ...
        if *s as libc::c_int == '#' as i32 &&
            ...
            if *s as libc::c_int == '}' as i32 { brackets -= 1 }
            let fresh5 = cp;
            cp = cp.offset(1);
            *fresh5 = *s
        }
...
/* Skip until end. */
#[no_mangle]
pub unsafe extern "C" fn format_skip(mut s: *const libc::c_char,
 ...
 -> *const libc::c_char {
    ...
    while *s as libc::c_int != '\u{0}' as i32 {
        ...
        if *s as libc::c_int == '#' as i32 &&
            ...
            if *s as libc::c_int == '}' as i32 { brackets -= 1 }
            if !strchr(end, *s as libc::c_int).is_null() &&
    ...
    if *s as libc::c_int == '\u{0}' as i32 { return 0 as *const libc::c_char }
    return s;
}
/* Return left and right alternatives separated by commas. */
unsafe extern "C" fn format_choose(mut es: *mut format_expand_state,
    ...
    if cp.is_null() { return -(1 as libc::c_int) }
...
unsafe extern "C" fn format_add_modifier(mut list: *mut *mut format_modifier,
                                         mut count: *mut u_int,
    ...
    *list =
        xreallocarray(*list as *mut libc::c_void,
                      (*count).wrapping_add(1 as libc::c_int as libc::c_uint)
                          as size_t,
                      ::std::mem::size_of::<format_modifier>() as
                          libc::c_ulong) as *mut format_modifier;
    let fresh6 = *count;
    *count = (*count).wrapping_add(1);
    fm = &mut *(*list).offset(fresh6 as isize) as *mut format_modifier;
    memcpy((*fm).modifier.as_mut_ptr() as *mut libc::c_void,
           c as *const libc::c_void, n);
...
/* Free modifier list. */
unsafe extern "C" fn format_free_modifiers(mut list: *mut format_modifier,
                                           mut count: u_int) {
    let mut i: u_int = 0;
    i = 0 as libc::c_int as u_int;
    while i < count {
        cmd_free_argv((*list.offset(i as isize)).argc,
...
/* Build modifier list. */
unsafe extern "C" fn format_build_modifiers(mut es: *mut format_expand_state,
 ...
 -> *mut format_modifier {
    ...
    *count = 0 as libc::c_int as u_int;
    while *cp as libc::c_int != '\u{0}' as i32 &&
        ...
        if *cp as libc::c_int == ';' as i32 { cp = cp.offset(1) }
        /* Check single character modifiers with no arguments. */
        if !strchr(b"lbdnqETSWP<>\x00" as *const u8 as *const libc::c_char,
                   *cp.offset(0 as libc::c_int as isize) as
                       libc::c_int).is_null() &&
               format_is_end(*cp.offset(1 as libc::c_int as isize)) != 0 {
            format_add_modifier(&mut list, count, cp,
                                1 as libc::c_int as size_t,
                                0 as *mut *mut libc::c_char,
                                0 as libc::c_int);
            cp = cp.offset(1)
        } else if (memcmp(b"||\x00" as *const u8 as *const libc::c_char as
                              *const libc::c_void, cp as *const libc::c_void,
                          2 as libc::c_int as libc::c_ulong) ==
                       0 as libc::c_int ||
                       memcmp(b"&&\x00" as *const u8 as *const libc::c_char as
                                  *const libc::c_void,
                       ...
                       memcmp(b">=\x00" as *const u8 as *const libc::c_char as
                              ...
                              2 as libc::c_int as libc::c_ulong) ==
                           0 as libc::c_int) &&
                      format_is_end(*cp.offset(2 as libc::c_int as isize)) !=
                          0 {
            format_add_modifier(&mut list, count, cp,
            ...
            if strchr(b"mCst=pe\x00" as *const u8 as *const libc::c_char,
                      *cp.offset(0 as libc::c_int as isize) as
                          libc::c_int).is_null() {
                break ;
            }
            ...
            if format_is_end(*cp.offset(1 as libc::c_int as isize)) != 0 {
                ...
                if *(*__ctype_b_loc()).offset(*cp.offset(1 as libc::c_int as
                                                             isize) as
                                                  libc::c_int as isize) as
                       libc::c_int &
                       _ISpunct as libc::c_int as libc::c_ushort as
                           libc::c_int == 0 ||
                       *cp.offset(1 as libc::c_int as isize) as libc::c_int ==
                           '-' as i32 {
                    end =
                        format_skip(cp.offset(1 as libc::c_int as isize),
                                    b":;\x00" as *const u8 as
                                        *const libc::c_char);
                    ...
                    value =
                        xstrndup(cp.offset(1 as libc::c_int as isize),
                                 end.wrapping_offset_from(cp.offset(1 as
                                                                        libc::c_int
                                                                        as
                                                                        isize))
                    ...
                    *fresh7 = format_expand1(es, value);
                    ...
                    format_add_modifier(&mut list, count, &mut c,
                                        1 as libc::c_int as size_t, argv,
                                        argc);
                    cp = end
                } else {
                    ...
                    cp = cp.offset(1);
                    loop  {
                        if *cp.offset(0 as libc::c_int as isize) as
                               libc::c_int ==
                               last[0 as libc::c_int as usize] as libc::c_int
                               &&
                               format_is_end(*cp.offset(1 as libc::c_int as
                                                            isize)) != 0 {
                            cp = cp.offset(1);
                            break ;
                        } else {
                            end =
                                format_skip(cp.offset(1 as libc::c_int as
                                                          isize),
                                            last.as_mut_ptr());
                            if end.is_null() { break ; }
                            cp = cp.offset(1);
                            ...
                            value =
                                xstrndup(cp,
                                         end.wrapping_offset_from(cp) as
                            ...
                            *fresh9 = format_expand1(es, value);
                            free(value as *mut libc::c_void);
                            cp = end;
                            if !(format_is_end(*cp.offset(0 as libc::c_int as
                                                              isize)) == 0) {
                                break ;
                            }
                        }
...
unsafe extern "C" fn format_match(mut fm: *mut format_modifier,
 ...
 -> *mut libc::c_char {
    ...
    if strchr(s, 'r' as i32).is_null() {
        ...
        if regexec(&mut r, text, 0 as libc::c_int as size_t,
...
unsafe extern "C" fn format_sub(mut fm: *mut format_modifier,
                                mut text: *const libc::c_char,
                                mut pattern: *const libc::c_char,
 ...
 -> *mut libc::c_char {
    ...
    if (*fm).argc >= 3 as libc::c_int &&
           !strchr(*(*fm).argv.offset(2 as libc::c_int as isize),
                   'i' as i32).is_null() {
        flags |= (1 as libc::c_int) << 1 as libc::c_int
    }
    value = regsub(pattern, with, text, flags);
    if value.is_null() { return xstrdup(text) }
    return value;
}
/* Search inside pane. */
unsafe extern "C" fn format_search(mut fm: *mut format_modifier,
 ...
 -> *mut libc::c_char {
    ...
    let mut value: *mut libc::c_char = 0 as *mut libc::c_char;
    if (*fm).argc >= 1 as libc::c_int {
        if !strchr(*(*fm).argv.offset(0 as libc::c_int as isize),
                   'i' as i32).is_null() {
...
/* Loop over sessions. */
unsafe extern "C" fn format_loop_sessions(mut es: *mut format_expand_state,
                                          mut fmt: *const libc::c_char)
 -> *mut libc::c_char {
    let mut ft: *mut format_tree = (*es).ft;
    ...
    value =
        xcalloc(1 as libc::c_int as size_t, 1 as libc::c_int as size_t) as
            *mut libc::c_char;
    ...
    while !s.is_null() {
        format_log1(es,
                    (*::std::mem::transmute::<&[u8; 21],
                                              &[libc::c_char; 21]>(b"format_loop_sessions\x00")).as_ptr(),
                    b"session loop: $%u\x00" as *const u8 as
                        *const libc::c_char, (*s).id);
        nft = format_create(c, item, 0 as libc::c_int, (*ft).flags);
        format_defaults(next.ft, (*ft).c, s, 0 as *mut winlink,
        ...
        format_free(next.ft);
        valuelen =
            (valuelen as libc::c_ulong).wrapping_add(strlen(expanded)) as
                size_t as size_t;
        value =
            xrealloc(value as *mut libc::c_void, valuelen) as
                *mut libc::c_char;
...
/* Loop over windows. */
unsafe extern "C" fn format_loop_windows(mut es: *mut format_expand_state,
                                         mut fmt: *const libc::c_char)
 -> *mut libc::c_char {
    let mut ft: *mut format_tree = (*es).ft;
    let mut c: *mut client = (*ft).client;
    ...
    if (*ft).s.is_null() {
        format_log1(es,
                    (*::std::mem::transmute::<&[u8; 20],
                                              &[libc::c_char; 20]>(b"format_loop_windows\x00")).as_ptr(),
                    b"window loop but no session\x00" as *const u8 as
                        *const libc::c_char);
        return 0 as *mut libc::c_char
    }
    if format_choose(es, fmt, &mut all, &mut active, 0 as libc::c_int) !=
           0 as libc::c_int {
        all = xstrdup(fmt);
    ...
    while !wl.is_null() {
        ...
        format_log1(es,
                    (*::std::mem::transmute::<&[u8; 20],
                                              &[libc::c_char; 20]>(b"format_loop_windows\x00")).as_ptr(),
                    b"window loop: %u @%u\x00" as *const u8 as
                        *const libc::c_char, (*wl).idx, (*w).id);
        if !active.is_null() && wl == (*(*ft).s).curw {
            use_0 = active
        } else { use_0 = all }
        nft =
            format_create(c, item,
                          (0x40000000 as libc::c_uint | (*w).id) as
                              libc::c_int, (*ft).flags);
...
/* Loop over panes. */
unsafe extern "C" fn format_loop_panes(mut es: *mut format_expand_state,
                                       mut fmt: *const libc::c_char)
 -> *mut libc::c_char {
    let mut ft: *mut format_tree = (*es).ft;
    let mut c: *mut client = (*ft).client;
    ...
    if (*ft).w.is_null() {
        format_log1(es,
                    (*::std::mem::transmute::<&[u8; 18],
                                              &[libc::c_char; 18]>(b"format_loop_panes\x00")).as_ptr(),
                    b"pane loop but no window\x00" as *const u8 as
                        *const libc::c_char);
        return 0 as *mut libc::c_char
    }
    if format_choose(es, fmt, &mut all, &mut active, 0 as libc::c_int) !=
    ...
    while !wp.is_null() {
        format_log1(es,
                    (*::std::mem::transmute::<&[u8; 18],
                                              &[libc::c_char; 18]>(b"format_loop_panes\x00")).as_ptr(),
                    b"pane loop: %%%u\x00" as *const u8 as
        ...
        nft =
            format_create(c, item,
                          (0x80000000 as libc::c_uint | (*wp).id) as
                              libc::c_int, (*ft).flags);
...
unsafe extern "C" fn format_replace_expression(mut mexp: *mut format_modifier,
                                               mut es:
                                                   *mut format_expand_state,
 ...
 -> *mut libc::c_char {
    ...
    if strcmp(*(*mexp).argv.offset(0 as libc::c_int as isize),
              b"+\x00" as *const u8 as *const libc::c_char) ==
           0 as libc::c_int {
        operator = ADD;
        current_block = 11307063007268554308;
    } else if strcmp(*(*mexp).argv.offset(0 as libc::c_int as isize),
                     b"-\x00" as *const u8 as *const libc::c_char) ==
                  0 as libc::c_int {
        operator = SUBTRACT;
        current_block = 11307063007268554308;
        ...
        current_block = 11307063007268554308;
                     ...
                     b"%\x00" as *const u8 as *const libc::c_char) ==
                  0 as libc::c_int ||
                  strcmp(*(*mexp).argv.offset(0 as libc::c_int as isize),
        ...
        format_log1(es,
                    (*::std::mem::transmute::<&[u8; 26],
                                              &[libc::c_char; 26]>(b"format_replace_expression\x00")).as_ptr(),
                    b"expression has no valid operator: \'%s\'\x00" as
    ...
    match current_block {
        11307063007268554308 => {
            ...
            if argc >= 2 as libc::c_int &&
                   !strchr(*(*mexp).argv.offset(1 as libc::c_int as isize),
                           'f' as i32).is_null() {
            ...
            if argc >= 3 as libc::c_int {
                prec =
                    strtonum(*(*mexp).argv.offset(2 as libc::c_int as isize),
                ...
                if !errstr.is_null() {
                    format_log1(es,
                                (*::std::mem::transmute::<&[u8; 26],
                                                          &[libc::c_char; 26]>(b"format_replace_expression\x00")).as_ptr(),
                                b"expression precision %s: %s\x00" as
                                    *const u8 as *const libc::c_char, errstr,
                                *(*mexp).argv.offset(2 as libc::c_int as
                                                         isize));
                    current_block = 2045625362422199595;
            ...
            match current_block {
                2045625362422199595 => { }
                _ => {
                    if format_choose(es, copy, &mut left, &mut right,
                                     1 as libc::c_int) != 0 as libc::c_int {
                        format_log1(es,
                                    (*::std::mem::transmute::<&[u8; 26],
                                                              &[libc::c_char; 26]>(b"format_replace_expression\x00")).as_ptr(),
                                    b"expression syntax error\x00" as
                                        *const u8 as *const libc::c_char);
                    } else {
                        ...
                        if *endch as libc::c_int != '\u{0}' as i32 {
                            format_log1(es,
                                        (*::std::mem::transmute::<&[u8; 26],
                                                                  &[libc::c_char; 26]>(b"format_replace_expression\x00")).as_ptr(),
                                        b"expression left side is invalid: %s\x00"
                                            as *const u8 as
                                            *const libc::c_char, left);
                        } else {
                            ...
                            if *endch as libc::c_int != '\u{0}' as i32 {
                                format_log1(es,
                                            (*::std::mem::transmute::<&[u8; 26],
                                                                      &[libc::c_char; 26]>(b"format_replace_expression\x00")).as_ptr(),
                                            b"expression right side is invalid: %s\x00"
                                                as *const u8 as
                                                *const libc::c_char, right);
                            } else {
                                if use_fp == 0 {
                                    mleft =
                                        mleft as libc::c_longlong as
                                            libc::c_double;
                                    mright =
                                        mright as libc::c_longlong as
                                            libc::c_double
                                }
                                format_log1(es,
                                            (*::std::mem::transmute::<&[u8; 26],
                                                                      &[libc::c_char; 26]>(b"format_replace_expression\x00")).as_ptr(),
                                            b"expression left side is: %.*f\x00"
                                                as *const u8 as
                                                *const libc::c_char, prec,
                                            mleft);
                                format_log1(es,
                                            (*::std::mem::transmute::<&[u8; 26],
                                                                      &[libc::c_char; 26]>(b"format_replace_expression\x00")).as_ptr(),
                                            b"expression right side is:  %.*f\x00"
                                                as *const u8 as
                                                *const libc::c_char, prec,
                                            mright);
                                match operator as libc::c_uint {
                                    ...
                                    7 => {
                                        result =
                                            (mleft > mright) as libc::c_int as
                                                libc::c_double
                                    }
                                    8 => {
                                        result =
                                            (mleft >= mright) as libc::c_int
                                                as libc::c_double
                                    }
                                    9 => {
                                        result =
                                ...
                                if use_fp != 0 {
                                    ...
                                    xasprintf(&mut value as
                                              ...
                                              b"%.*f\x00" as *const u8 as
                                                  *const libc::c_char, prec,
                                              result as libc::c_longlong as
                                                  libc::c_double);
                                }
                                format_log1(es,
                                            (*::std::mem::transmute::<&[u8; 26],
                                                                      &[libc::c_char; 26]>(b"format_replace_expression\x00")).as_ptr(),
                                            b"expression result is %s\x00" as
                                                *const u8 as
...
/* Replace a key. */
unsafe extern "C" fn format_replace(mut es: *mut format_expand_state,
                                    mut key: *const libc::c_char,
                                    mut keylen: size_t,
    ...
    while i < count {
        ...
        if format_logging(ft) != 0 {
            format_log1(es,
                        (*::std::mem::transmute::<&[u8; 15],
                                                  &[libc::c_char; 15]>(b"format_replace\x00")).as_ptr(),
                        b"modifier %u is %s\x00" as *const u8 as
                            *const libc::c_char, i,
            ...
            while j < (*fm).argc {
                format_log1(es,
                            (*::std::mem::transmute::<&[u8; 15],
                                                      &[libc::c_char; 15]>(b"format_replace\x00")).as_ptr(),
                            b"modifier %u argument %d: %s\x00" as *const u8 as
        ...
        if (*fm).size == 1 as libc::c_int as libc::c_uint {
            match (*fm).modifier[0 as libc::c_int as usize] as libc::c_int {
                ...
                115 => {
                    if !((*fm).argc < 2 as libc::c_int) {
                        sub =
                            xreallocarray(sub as *mut libc::c_void,
                                          nsub.wrapping_add(1 as libc::c_int
                                                                as
                                                                libc::c_uint)
                                              as size_t,
                                          ::std::mem::size_of::<*mut format_modifier>()
                                              as libc::c_ulong) as
                                *mut *mut format_modifier;
                        let fresh10 = nsub;
                ...
                61 => {
                    if !((*fm).argc < 1 as libc::c_int) {
                        limit =
                            strtonum(*(*fm).argv.offset(0 as libc::c_int as
                                                            isize),
                                     (-(2147483647 as libc::c_int) -
                                          1 as libc::c_int) as
                                         libc::c_longlong,
                                     2147483647 as libc::c_int as
                                         libc::c_longlong, &mut errptr) as
                                libc::c_int;
                        if !errptr.is_null() { limit = 0 as libc::c_int }
                        if (*fm).argc >= 2 as libc::c_int &&
                               !(*(*fm).argv.offset(1 as libc::c_int as
                                                        isize)).is_null() {
                            marker =
                                *(*fm).argv.offset(1 as libc::c_int as isize)
                        }
                    }
                }
                112 => {
                        ...
                        width =
                            strtonum(*(*fm).argv.offset(0 as libc::c_int as
                                     ...
                                     2147483647 as libc::c_int as
                                         libc::c_longlong, &mut errptr) as
                                libc::c_int;
                        if !errptr.is_null() { width = 0 as libc::c_int }
                    }
                }
                101 => {
                    if !((*fm).argc < 1 as libc::c_int ||
                             (*fm).argc > 3 as libc::c_int) {
                        mexp = fm
                    }
                }
                ...
                116 => {
                    ...
                    if !((*fm).argc < 1 as libc::c_int) {
                        if !strchr(*(*fm).argv.offset(0 as libc::c_int as
                                                          isize),
                                   'p' as i32).is_null() {
                            modifiers |= 0x400 as libc::c_int
                        } else if (*fm).argc >= 2 as libc::c_int &&
                                      !strchr(*(*fm).argv.offset(0 as
                                                                     libc::c_int
                                                                     as
                                                                     isize),
                                              'f' as i32).is_null() {
                            time_format =
                                format_strip(*(*fm).argv.offset(1 as
                                                                    libc::c_int
                                                                    as isize))
                        }
                    }
            ...
            if strcmp((*fm).modifier.as_mut_ptr(),
                      b"||\x00" as *const u8 as *const libc::c_char) ==
                   0 as libc::c_int ||
                   strcmp((*fm).modifier.as_mut_ptr(),
                          b"&&\x00" as *const u8 as *const libc::c_char) ==
                       0 as libc::c_int ||
                   strcmp((*fm).modifier.as_mut_ptr(),
                          b"==\x00" as *const u8 as *const libc::c_char) ==
    ...
    if modifiers & 0x10 as libc::c_int != 0 {
        ...
        if modifiers & 0x80 as libc::c_int != 0 {
            ...
            } else { current_block = 10449397189332721644; }
            ...
            if wp.is_null() {
                format_log1(es,
                            (*::std::mem::transmute::<&[u8; 15],
                                                      &[libc::c_char; 15]>(b"format_replace\x00")).as_ptr(),
                            b"search \'%s\' but no pane\x00" as *const u8 as
                                *const libc::c_char, new);
                value = xstrdup(b"0\x00" as *const u8 as *const libc::c_char)
            } else {
                format_log1(es,
                            (*::std::mem::transmute::<&[u8; 15],
                                                      &[libc::c_char; 15]>(b"format_replace\x00")).as_ptr(),
                            b"search \'%s\' pane %%%u\x00" as *const u8 as
                                *const libc::c_char, new, (*wp).id);
            ...
            if format_choose(es, copy, &mut left, &mut right,
                ...
                format_log1(es,
                            (*::std::mem::transmute::<&[u8; 15],
                                                      &[libc::c_char; 15]>(b"format_replace\x00")).as_ptr(),
                            b"compare %s syntax error: %s\x00" as *const u8 as
                                *const libc::c_char,
                            (*cmp).modifier.as_mut_ptr(), copy);
                current_block = 4023792313809996375;
            } else {
                format_log1(es,
                            (*::std::mem::transmute::<&[u8; 15],
                                                      &[libc::c_char; 15]>(b"format_replace\x00")).as_ptr(),
                            b"compare %s left is: %s\x00" as *const u8 as
                                *const libc::c_char,
                            (*cmp).modifier.as_mut_ptr(), left);
                format_log1(es,
                            (*::std::mem::transmute::<&[u8; 15],
                                                      &[libc::c_char; 15]>(b"format_replace\x00")).as_ptr(),
                            b"compare %s right is: %s\x00" as *const u8 as
                                *const libc::c_char,
                            (*cmp).modifier.as_mut_ptr(), right);
                if strcmp((*cmp).modifier.as_mut_ptr(),
                          b"||\x00" as *const u8 as *const libc::c_char) ==
                       0 as libc::c_int {
                    if format_true(left) != 0 || format_true(right) != 0 {
                        value =
                            xstrdup(b"1\x00" as *const u8 as
                                        *const libc::c_char)
                    } else {
                        value =
                            xstrdup(b"0\x00" as *const u8 as
                                        *const libc::c_char)
                    }
                } else if strcmp((*cmp).modifier.as_mut_ptr(),
                                 b"&&\x00" as *const u8 as
            ...
            cp =
                format_skip(copy.offset(1 as libc::c_int as isize),
                            b",\x00" as *const u8 as *const libc::c_char);
            if cp.is_null() {
                format_log1(es,
                            (*::std::mem::transmute::<&[u8; 15],
                                                      &[libc::c_char; 15]>(b"format_replace\x00")).as_ptr(),
                            b"condition syntax error: %s\x00" as *const u8 as
                                *const libc::c_char,
                            copy.offset(1 as libc::c_int as isize));
                current_block = 4023792313809996375;
            } else {
                condition =
                    xstrndup(copy.offset(1 as libc::c_int as isize),
                             cp.wrapping_offset_from(copy.offset(1 as
                                                                     libc::c_int
                                                                     as
                                                                     isize))
                                 as libc::c_long as size_t);
                format_log1(es,
                            (*::std::mem::transmute::<&[u8; 15],
                                                      &[libc::c_char; 15]>(b"format_replace\x00")).as_ptr(),
                            b"condition is: %s\x00" as *const u8 as
                                *const libc::c_char, condition);
			 ...
			 */
                    ...
                    if strcmp(found, condition) == 0 as libc::c_int {
                        ...
                        format_log1(es,
                                    (*::std::mem::transmute::<&[u8; 15],
                                                              &[libc::c_char; 15]>(b"format_replace\x00")).as_ptr(),
                                    b"condition \'%s\' not found; assuming false\x00"
                    ...
                    format_log1(es,
                                (*::std::mem::transmute::<&[u8; 15],
                                                          &[libc::c_char; 15]>(b"format_replace\x00")).as_ptr(),
                                b"condition \'%s\' found\x00" as *const u8 as
                                    *const libc::c_char, condition);
                }
                if format_choose(es, cp.offset(1 as libc::c_int as isize),
                                 &mut left, &mut right, 0 as libc::c_int) !=
                       0 as libc::c_int {
                    format_log1(es,
                                (*::std::mem::transmute::<&[u8; 15],
                                                          &[libc::c_char; 15]>(b"format_replace\x00")).as_ptr(),
                                b"condition \'%s\' syntax error: %s\x00" as
                                    *const u8 as *const libc::c_char,
                                condition,
                                cp.offset(1 as libc::c_int as isize));
                    free(found as *mut libc::c_void);
            ...
            if !mexp.is_null() {
                ...
                format_log1(es,
                            (*::std::mem::transmute::<&[u8; 15],
                                                      &[libc::c_char; 15]>(b"format_replace\x00")).as_ptr(),
                            b"expanding inner format \'%s\'\x00" as *const u8
                                as *const libc::c_char, copy);
                ...
                if value.is_null() {
                    format_log1(es,
                                (*::std::mem::transmute::<&[u8; 15],
                                                          &[libc::c_char; 15]>(b"format_replace\x00")).as_ptr(),
                                b"format \'%s\' not found\x00" as *const u8 as
                    ...
                    format_log1(es,
                                (*::std::mem::transmute::<&[u8; 15],
                                                          &[libc::c_char; 15]>(b"format_replace\x00")).as_ptr(),
                                b"format \'%s\' found: %s\x00" as *const u8 as
                                    *const libc::c_char, copy, value);
                }
            }
            current_block = 10449397189332721644;
        }
        match current_block {
            ...
            _ => {
                format_log1(es,
                            (*::std::mem::transmute::<&[u8; 15],
                                                      &[libc::c_char; 15]>(b"format_replace\x00")).as_ptr(),
                            b"failed %s\x00" as *const u8 as
                                *const libc::c_char, copy0);
    ...
    while i < nsub {
        ...
        format_log1(es,
                    (*::std::mem::transmute::<&[u8; 15],
                                              &[libc::c_char; 15]>(b"format_replace\x00")).as_ptr(),
                    b"substitute \'%s\' to \'%s\': %s\x00" as *const u8 as
                        *const libc::c_char, left, right, new);
        free(value as *mut libc::c_void);
        value = new;
        free(right as *mut libc::c_void);
        free(left as *mut libc::c_void);
    ...
    if limit > 0 as libc::c_int {
        ...
        if !marker.is_null() && strcmp(new, value) != 0 as libc::c_int {
            free(value as *mut libc::c_void);
            xasprintf(&mut value as *mut *mut libc::c_char,
                      b"%s%s\x00" as *const u8 as *const libc::c_char, new,
                      marker);
        } else { free(value as *mut libc::c_void); value = new }
        format_log1(es,
                    (*::std::mem::transmute::<&[u8; 15],
                                              &[libc::c_char; 15]>(b"format_replace\x00")).as_ptr(),
                    b"applied length limit %d: %s\x00" as *const u8 as
                        *const libc::c_char, limit, value);
    } else if limit < 0 as libc::c_int {
        new = format_trim_right(value, -limit as u_int);
        if !marker.is_null() && strcmp(new, value) != 0 as libc::c_int {
            free(value as *mut libc::c_void);
            xasprintf(&mut value as *mut *mut libc::c_char,
                      b"%s%s\x00" as *const u8 as *const libc::c_char, marker,
                      new);
        } else { free(value as *mut libc::c_void); value = new }
        format_log1(es,
                    (*::std::mem::transmute::<&[u8; 15],
    ...
    if width > 0 as libc::c_int {
        ...
        format_log1(es,
                    (*::std::mem::transmute::<&[u8; 15],
                                              &[libc::c_char; 15]>(b"format_replace\x00")).as_ptr(),
                    b"applied padding width %d: %s\x00" as *const u8 as
                        *const libc::c_char, width, value);
    ...
    if modifiers & 0x800 as libc::c_int != 0 {
        ...
        format_log1(es,
                    (*::std::mem::transmute::<&[u8; 15],
                                              &[libc::c_char; 15]>(b"format_replace\x00")).as_ptr(),
                    b"replacing with length: %s\x00" as *const u8 as
                        *const libc::c_char, new);
    }
    /* Expand the buffer and copy in the value. */
    valuelen = strlen(value);
    while (*len).wrapping_sub(*off) <
              valuelen.wrapping_add(1 as libc::c_int as libc::c_ulong) {
        *buf =
            xreallocarray(*buf as *mut libc::c_void,
                          2 as libc::c_int as size_t, *len) as
                *mut libc::c_char;
        *len =
            (*len as
    ...
    *off = (*off as libc::c_ulong).wrapping_add(valuelen) as size_t as size_t;
    format_log1(es,
                (*::std::mem::transmute::<&[u8; 15],
                                          &[libc::c_char; 15]>(b"format_replace\x00")).as_ptr(),
                b"replaced \'%s\' with \'%s\'\x00" as *const u8 as
                    *const libc::c_char, copy0, value);
    free(value as *mut libc::c_void);
    free(sub as *mut libc::c_void);
    format_free_modifiers(list, count);
    free(copy0 as *mut libc::c_void);
...
unsafe extern "C" fn format_expand1(mut es: *mut format_expand_state,
                                    mut fmt: *const libc::c_char)
 -> *mut libc::c_char {
    let mut ft: *mut format_tree = (*es).ft;
    let mut buf: *mut libc::c_char = 0 as *mut libc::c_char;
    ...
    format_log1(es,
                (*::std::mem::transmute::<&[u8; 15],
                                          &[libc::c_char; 15]>(b"format_expand1\x00")).as_ptr(),
                b"expanding format: %s\x00" as *const u8 as
    ...
    if (*es).flags & 0x1 as libc::c_int != 0 {
        ...
        if strftime(expanded.as_mut_ptr(),
            ...
            format_log1(es,
                        (*::std::mem::transmute::<&[u8; 15],
                                                  &[libc::c_char; 15]>(b"format_expand1\x00")).as_ptr(),
                        b"format is too long\x00" as *const u8 as
                            *const libc::c_char);
            return xstrdup(b"\x00" as *const u8 as *const libc::c_char)
        }
        if format_logging(ft) != 0 &&
               strcmp(expanded.as_mut_ptr(), fmt) != 0 as libc::c_int {
            format_log1(es,
                        (*::std::mem::transmute::<&[u8; 15],
                                                  &[libc::c_char; 15]>(b"format_expand1\x00")).as_ptr(),
                        b"after time expanded: %s\x00" as *const u8 as
            ...
            match ch {
                40 => {
                    ...
                    while *ptr as libc::c_int != '\u{0}' as i32 {
                        if *ptr as libc::c_int == '(' as i32 { brackets += 1 }
                    ...
                    n =
                        ptr.wrapping_offset_from(fmt) as libc::c_long as
                            size_t;
                    name = xstrndup(fmt, n);
                    format_log1(es,
                                (*::std::mem::transmute::<&[u8; 15],
                                                          &[libc::c_char; 15]>(b"format_expand1\x00")).as_ptr(),
                                b"found #(): %s\x00" as *const u8 as
                    ...
                    if (*ft).flags & 0x4 as libc::c_int != 0 ||
                        ...
                        format_log1(es,
                                    (*::std::mem::transmute::<&[u8; 15],
                                                              &[libc::c_char; 15]>(b"format_expand1\x00")).as_ptr(),
                                    b"#() is disabled\x00" as *const u8 as
                                        *const libc::c_char);
                    } else {
                        ...
                        format_log1(es,
                                    (*::std::mem::transmute::<&[u8; 15],
                                                              &[libc::c_char; 15]>(b"format_expand1\x00")).as_ptr(),
                                    b"#() result: %s\x00" as *const u8 as
                                        *const libc::c_char, out);
                    }
                    free(name as *mut libc::c_void);
                    outlen = strlen(out);
                    while len.wrapping_sub(off) <
                              outlen.wrapping_add(1 as libc::c_int as
                                                      libc::c_ulong) {
                        buf =
                            xreallocarray(buf as *mut libc::c_void,
                                          2 as libc::c_int as size_t, len) as
                    ...
                    off =
                        (off as libc::c_ulong).wrapping_add(outlen) as size_t
                            as size_t;
                    free(out as *mut libc::c_void);
                    fmt =
                        fmt.offset(n.wrapping_add(1 as libc::c_int as
                                                      libc::c_ulong) as isize)
                }
                123 => {
                    ptr =
                        format_skip((fmt as
                                         *mut libc::c_char).offset(-(2 as
                                                                         libc::c_int
                                                                         as
                                                                         isize)),
                                    b"}\x00" as *const u8 as
                    ...
                    format_log1(es,
                                ...
                                fmt);
                    if format_replace(es, fmt, n, &mut buf, &mut len,
                                      &mut off) != 0 as libc::c_int {
                        break ;
                    }
                    fmt =
                        fmt.offset(n.wrapping_add(1 as libc::c_int as
                    ...
                    if s.is_null() {
                        ...
                        *buf.offset(fresh17 as isize) = ch as libc::c_char
                        ...
                        format_log1(es,
                                    (*::std::mem::transmute::<&[u8; 15],
                                                              &[libc::c_char; 15]>(b"format_expand1\x00")).as_ptr(),
                                    b"found #%c: %s\x00" as *const u8 as
                                        *const libc::c_char, ch, s);
                        if format_replace(es, s, n, &mut buf, &mut len,
                                          &mut off) != 0 as libc::c_int {
                            break ;
                        }
                    }
 ...
 -> *mut libc::c_char {
    let mut es: format_expand_state =
        format_expand_state{ft: 0 as *mut format_tree,
                            ...
                            time: 0,
                            flags: 0,};
    memset(&mut es as *mut format_expand_state as *mut libc::c_void,
           0 as libc::c_int,
...
#[no_mangle]
pub unsafe extern "C" fn format_single_from_state(mut item: *mut cmdq_item,
                                                  mut fmt:
                                                      *const libc::c_char,
                                                  mut c: *mut client,
...
#[no_mangle]
pub unsafe extern "C" fn format_single_from_target(mut item: *mut cmdq_item,
                                                   mut fmt:
                                                       *const libc::c_char)
 -> *mut libc::c_char {
...
#[no_mangle]
pub unsafe extern "C" fn format_create_defaults(mut item: *mut cmdq_item,
                                                mut c: *mut client,
                                                mut s: *mut session,
                                                mut wl: *mut winlink,
                                                mut wp: *mut window_pane)
 -> *mut format_tree {
    ...
    if !item.is_null() {
        ...
        ft =
            format_create(0 as *mut client, item, 0 as libc::c_int,
                          0 as libc::c_int)
    }
...
#[no_mangle]
pub unsafe extern "C" fn format_create_from_state(mut item: *mut cmdq_item,
                                                  mut c: *mut client,
                                                  mut fs: *mut cmd_find_state)
 -> *mut format_tree {
...
#[no_mangle]
pub unsafe extern "C" fn format_create_from_target(mut item: *mut cmdq_item)
 -> *mut format_tree {
    let mut tc: *mut client = cmdq_get_target_client(item);
...
pub unsafe extern "C" fn format_defaults(mut ft: *mut format_tree,
    ...
    if !c.is_null() && !(*c).name.is_null() {
        ...
        log_debug(b"%s: c=none\x00" as *const u8 as *const libc::c_char,
                  (*::std::mem::transmute::<&[u8; 16],
                                            &[libc::c_char; 16]>(b"format_defaults\x00")).as_ptr());
    }
    if !s.is_null() {
    ...
    if !wl.is_null() {
        log_debug(b"%s: wl=%u\x00" as *const u8 as *const libc::c_char,
                  (*::std::mem::transmute::<&[u8; 16],
                                            &[libc::c_char; 16]>(b"format_defaults\x00")).as_ptr(),
                  (*wl).idx);
    } else {
    ...
    format_add(ft, b"pane_format\x00" as *const u8 as *const libc::c_char,
               ...
               (wp != 0 as *mut libc::c_void as *mut window_pane) as
                   libc::c_int);
    if s.is_null() && !c.is_null() { s = (*c).session }
    if wl.is_null() && !s.is_null() { wl = (*s).curw }
    if wp.is_null() && !wl.is_null() { wp = (*(*wl).window).active }
    if !c.is_null() { format_defaults_client(ft, c); }
    if !s.is_null() { format_defaults_session(ft, s); }
    if !wl.is_null() { format_defaults_winlink(ft, wl); }
    if !wp.is_null() { format_defaults_pane(ft, wp); }
    pb = paste_get_top(0 as *mut *const libc::c_char);
    if !pb.is_null() { format_defaults_paste_buffer(ft, pb); };
}
...
unsafe extern "C" fn format_defaults_session(mut ft: *mut format_tree,
    ...
    format_add(ft, b"session_path\x00" as *const u8 as *const libc::c_char,
               b"%s\x00" as *const u8 as *const libc::c_char, (*s).cwd);
    ...
    if !sg.is_null() {
        ...
        format_add(ft,
                   b"session_group_attached\x00" as *const u8 as
                       *const libc::c_char,
                   b"%u\x00" as *const u8 as *const libc::c_char,
                   session_group_attached_count(sg));
        format_add(ft,
                   b"session_group_many_attached\x00" as *const u8 as
                       *const libc::c_char,
                   b"%u\x00" as *const u8 as *const libc::c_char,
                   (session_group_attached_count(sg) >
        ...
        format_add_cb(ft,
                      b"session_group_list\x00" as *const u8 as
                          *const libc::c_char,
                      Some(format_cb_session_group_list as
                               unsafe extern "C" fn(_: *mut format_tree)
                                   -> *mut libc::c_char));
        format_add_cb(ft,
                      b"session_group_attached_list\x00" as *const u8 as
                          *const libc::c_char,
                      Some(format_cb_session_group_attached_list as
                               unsafe extern "C" fn(_: *mut format_tree)
                                   -> *mut libc::c_char));
    }
    format_add_tv(ft,
    ...
    format_add_tv(ft,
                  b"session_last_attached\x00" as *const u8 as
                      *const libc::c_char, &mut (*s).last_attached_time);
    format_add_tv(ft,
    ...
    format_add(ft,
               b"session_many_attached\x00" as *const u8 as
                   *const libc::c_char,
               b"%d\x00" as *const u8 as *const libc::c_char,
               ((*s).attached > 1 as libc::c_int as libc::c_uint) as
                   libc::c_int);
    format_add_cb(ft,
                  b"session_attached_list\x00" as *const u8 as
                      *const libc::c_char,
                  Some(format_cb_session_attached_list as
                           unsafe extern "C" fn(_: *mut format_tree)
                               -> *mut libc::c_char));
    format_add_cb(ft,
                  b"session_alerts\x00" as *const u8 as *const libc::c_char,
                  Some(format_cb_session_alerts as
                           unsafe extern "C" fn(_: *mut format_tree)
                               -> *mut libc::c_char));
    format_add_cb(ft,
                  b"session_stack\x00" as *const u8 as *const libc::c_char,
                  Some(format_cb_session_stack as
                           unsafe extern "C" fn(_: *mut format_tree)
                               -> *mut libc::c_char));
...
unsafe extern "C" fn format_defaults_client(mut ft: *mut format_tree,
    ...
    if (*ft).s.is_null() { (*ft).s = (*c).session }
    (*ft).c = c;
    ...
    format_add(ft,
               ...
               b"%u\x00" as *const u8 as *const libc::c_char, (*tty).xpixel);
    format_add(ft,
               b"client_cell_height\x00" as *const u8 as *const libc::c_char,
               b"%u\x00" as *const u8 as *const libc::c_char, (*tty).ypixel);
    ...
    format_add(ft,
               ...
               b"%d\x00" as *const u8 as *const libc::c_char,
               ((*c).flags & 0x2000 as libc::c_int as libc::c_ulong != 0) as
                   libc::c_int);
    format_add(ft, b"client_termname\x00" as *const u8 as *const libc::c_char,
               b"%s\x00" as *const u8 as *const libc::c_char, (*c).term_name);
    format_add(ft,
    ...
    format_add(ft, b"client_flags\x00" as *const u8 as *const libc::c_char,
               b"%s\x00" as *const u8 as *const libc::c_char,
...
pub unsafe extern "C" fn format_defaults_window(mut ft: *mut format_tree,
    ...
    format_add(ft,
               ...
               b"%u\x00" as *const u8 as *const libc::c_char, (*w).xpixel);
    format_add(ft,
               b"window_cell_height\x00" as *const u8 as *const libc::c_char,
               b"%u\x00" as *const u8 as *const libc::c_char, (*w).ypixel);
    format_add_cb(ft,
                  b"window_layout\x00" as *const u8 as *const libc::c_char,
                  Some(format_cb_window_layout as
                           unsafe extern "C" fn(_: *mut format_tree)
                               -> *mut libc::c_char));
    format_add_cb(ft,
                  b"window_visible_layout\x00" as *const u8 as
                      *const libc::c_char,
                  Some(format_cb_window_visible_layout as
                           unsafe extern "C" fn(_: *mut format_tree)
                               -> *mut libc::c_char));
...
unsafe extern "C" fn format_defaults_winlink(mut ft: *mut format_tree,
    ...
    let mut sx: u_int = 0;
    let mut sy: u_int = 0;
    if (*ft).w.is_null() { format_defaults_window(ft, w); }
    (*ft).wl = wl;
    if !c.is_null() {
        flag =
            tty_window_offset(&mut (*c).tty, &mut ox, &mut oy, &mut sx,
                              &mut sy);
        format_add(ft,
        ...
        if flag != 0 {
            format_add(ft,
                       b"window_offset_x\x00" as *const u8 as
                           *const libc::c_char,
                       b"%u\x00" as *const u8 as *const libc::c_char, ox);
            format_add(ft,
                       b"window_offset_y\x00" as *const u8 as
                           *const libc::c_char,
                       b"%u\x00" as *const u8 as *const libc::c_char, oy);
        }
    }
    ...
    format_add_cb(ft,
                  b"window_stack_index\x00" as *const u8 as
                      *const libc::c_char,
                  Some(format_cb_window_stack_index as
                           unsafe extern "C" fn(_: *mut format_tree)
                               -> *mut libc::c_char));
    ...
    format_add_cb(ft,
                  b"window_active_sessions\x00" as *const u8 as
                      *const libc::c_char,
                  Some(format_cb_window_active_sessions as
                           unsafe extern "C" fn(_: *mut format_tree)
                               -> *mut libc::c_char));
    format_add_cb(ft,
                  b"window_active_sessions_list\x00" as *const u8 as
                      *const libc::c_char,
                  Some(format_cb_window_active_sessions_list as
                           unsafe extern "C" fn(_: *mut format_tree)
                               -> *mut libc::c_char));
    format_add_cb(ft,
                  b"window_active_clients\x00" as *const u8 as
                      *const libc::c_char,
                  Some(format_cb_window_active_clients as
                           unsafe extern "C" fn(_: *mut format_tree)
                               -> *mut libc::c_char));
    format_add_cb(ft,
                  b"window_active_clients_list\x00" as *const u8 as
                      *const libc::c_char,
                  Some(format_cb_window_active_clients_list as
                           unsafe extern "C" fn(_: *mut format_tree)
                               -> *mut libc::c_char));
    format_add(ft,
    ...
    format_add(ft, b"window_end_flag\x00" as *const u8 as *const libc::c_char,
               b"%d\x00" as *const u8 as *const libc::c_char,
               (wl == winlinks_RB_MINMAX(&mut (*s).windows, 1 as libc::c_int))
                   as libc::c_int);
    if server_check_marked() != 0 && marked_pane.wl == wl {
        format_add(ft,
                   b"window_marked_flag\x00" as *const u8 as
                       *const libc::c_char,
                   b"1\x00" as *const u8 as *const libc::c_char);
    } else {
        format_add(ft,
    ...
    format_add_cb(ft,
                  b"window_linked_sessions_list\x00" as *const u8 as
                      *const libc::c_char,
                  Some(format_cb_window_linked_sessions_list as
                           unsafe extern "C" fn(_: *mut format_tree)
                               -> *mut libc::c_char));
    format_add(ft,
               b"window_linked_sessions\x00" as *const u8 as
                   *const libc::c_char,
               b"%u\x00" as *const u8 as *const libc::c_char,
...
pub unsafe extern "C" fn format_defaults_pane(mut ft: *mut format_tree,
    ...
    format_add_cb(ft,
                  ...
                  Some(format_cb_history_bytes as
                           unsafe extern "C" fn(_: *mut format_tree)
                               -> *mut libc::c_char));
    format_add_cb(ft,
                  b"history_all_bytes\x00" as *const u8 as
                      *const libc::c_char,
                  Some(format_cb_history_all_bytes as
                           unsafe extern "C" fn(_: *mut format_tree)
                               -> *mut libc::c_char));
    format_add(ft, b"pane_written\x00" as *const u8 as *const libc::c_char,
               b"%zu\x00" as *const u8 as *const libc::c_char, (*wp).written);
    format_add(ft, b"pane_skipped\x00" as *const u8 as *const libc::c_char,
               b"%zu\x00" as *const u8 as *const libc::c_char, (*wp).skipped);
    ...
    if !(*wp).base.path.is_null() {
        format_add(ft, b"pane_path\x00" as *const u8 as *const libc::c_char,
                   b"%s\x00" as *const u8 as *const libc::c_char,
                   (*wp).base.path);
    }
    ...
    format_add(ft, b"pane_input_off\x00" as *const u8 as *const libc::c_char,
               b"%d\x00" as *const u8 as *const libc::c_char,
    ...
    format_add(ft, b"pane_last\x00" as *const u8 as *const libc::c_char,
               b"%d\x00" as *const u8 as *const libc::c_char,
               (wp == (*w).last) as libc::c_int);
    if server_check_marked() != 0 && marked_pane.wp == wp {
        format_add(ft, b"pane_marked\x00" as *const u8 as *const libc::c_char,
                   b"1\x00" as *const u8 as *const libc::c_char);
    } else {
        format_add(ft, b"pane_marked\x00" as *const u8 as *const libc::c_char,
                   b"0\x00" as *const u8 as *const libc::c_char);
    }
    format_add(ft, b"pane_marked_set\x00" as *const u8 as *const libc::c_char,
               b"%d\x00" as *const u8 as *const libc::c_char,
               server_check_marked());
    format_add(ft, b"pane_left\x00" as *const u8 as *const libc::c_char,
               b"%u\x00" as *const u8 as *const libc::c_char, (*wp).xoff);
    format_add(ft, b"pane_top\x00" as *const u8 as *const libc::c_char,
               b"%u\x00" as *const u8 as *const libc::c_char, (*wp).yoff);
    format_add(ft, b"pane_right\x00" as *const u8 as *const libc::c_char,
               b"%u\x00" as *const u8 as *const libc::c_char,
               (*wp).xoff.wrapping_add((*wp).sx).wrapping_sub(1 as libc::c_int
                                                                  as
                                                                  libc::c_uint));
    format_add(ft, b"pane_bottom\x00" as *const u8 as *const libc::c_char,
               b"%u\x00" as *const u8 as *const libc::c_char,
               (*wp).yoff.wrapping_add((*wp).sy).wrapping_sub(1 as libc::c_int
                                                                  as
                                                                  libc::c_uint));
    format_add(ft, b"pane_at_left\x00" as *const u8 as *const libc::c_char,
               b"%d\x00" as *const u8 as *const libc::c_char,
               ((*wp).xoff == 0 as libc::c_int as libc::c_uint) as
                   libc::c_int);
    format_add_cb(ft, b"pane_at_top\x00" as *const u8 as *const libc::c_char,
                  Some(format_cb_pane_at_top as
                           unsafe extern "C" fn(_: *mut format_tree)
                               -> *mut libc::c_char));
    format_add(ft, b"pane_at_right\x00" as *const u8 as *const libc::c_char,
               b"%d\x00" as *const u8 as *const libc::c_char,
               ((*wp).xoff.wrapping_add((*wp).sx) == (*w).sx) as libc::c_int);
    format_add_cb(ft,
                  b"pane_at_bottom\x00" as *const u8 as *const libc::c_char,
                  Some(format_cb_pane_at_bottom as
                           unsafe extern "C" fn(_: *mut format_tree)
                               -> *mut libc::c_char));
    wme = (*wp).modes.tqh_first;
    if !wme.is_null() {
        format_add(ft, b"pane_mode\x00" as *const u8 as *const libc::c_char,
                   b"%s\x00" as *const u8 as *const libc::c_char,
                   (*(*wme).mode).name);
        if (*(*wme).mode).formats.is_some() {
            (*(*wme).mode).formats.expect("non-null function pointer")(wme,
                                                                       ft);
        }
    }
    format_add_cb(ft, b"pane_in_mode\x00" as *const u8 as *const libc::c_char,
                  Some(format_cb_pane_in_mode as
                           unsafe extern "C" fn(_: *mut format_tree)
                               -> *mut libc::c_char));
    format_add(ft,
    ...
    format_add(ft,
               ...
               b"%u\x00" as *const u8 as *const libc::c_char,
               (*wp).base.rlower);
    format_add(ft, b"alternate_on\x00" as *const u8 as *const libc::c_char,
               b"%d\x00" as *const u8 as *const libc::c_char,
               ((*wp).base.saved_grid != 0 as *mut libc::c_void as *mut grid)
                   as libc::c_int);
    if (*wp).base.saved_cx !=
           (2147483647 as libc::c_int as
    ...
    format_add(ft, b"origin_flag\x00" as *const u8 as *const libc::c_char,
               b"%d\x00" as *const u8 as *const libc::c_char,
               ((*wp).base.mode & 0x2000 as libc::c_int != 0) as libc::c_int);
    format_add(ft, b"mouse_any_flag\x00" as *const u8 as *const libc::c_char,
               b"%d\x00" as *const u8 as *const libc::c_char,
               ((*wp).base.mode &
    ...
    format_add(ft, b"mouse_all_flag\x00" as *const u8 as *const libc::c_char,
               b"%d\x00" as *const u8 as *const libc::c_char,
               ((*wp).base.mode & 0x1000 as libc::c_int != 0) as libc::c_int);
    format_add(ft, b"mouse_utf8_flag\x00" as *const u8 as *const libc::c_char,
               b"%d\x00" as *const u8 as *const libc::c_char,
               ((*wp).base.mode & 0x100 as libc::c_int != 0) as libc::c_int);
    format_add(ft, b"mouse_sgr_flag\x00" as *const u8 as *const libc::c_char,
               b"%d\x00" as *const u8 as *const libc::c_char,
               ((*wp).base.mode & 0x200 as libc::c_int != 0) as libc::c_int);
    format_add_cb(ft, b"pane_tabs\x00" as *const u8 as *const libc::c_char,
                  Some(format_cb_pane_tabs as
                           unsafe extern "C" fn(_: *mut format_tree)
                               -> *mut libc::c_char));
}
