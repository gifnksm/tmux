 ...
 */
unsafe extern "C" fn regsub_copy(mut buf: *mut *mut libc::c_char,
                                 mut len: *mut size_t,
    ...
    *buf =
        xrealloc(*buf as *mut libc::c_void,
                 (*len).wrapping_add(add).wrapping_add(1 as libc::c_int as
                                                           libc::c_ulong)) as
            *mut libc::c_char;
    memcpy((*buf).offset(*len as isize) as *mut libc::c_void,
           text.offset(start as isize) as *const libc::c_void, add);
    *len = (*len as libc::c_ulong).wrapping_add(add) as size_t as size_t;
}
unsafe extern "C" fn regsub_expand(mut buf: *mut *mut libc::c_char,
                                   mut len: *mut size_t,
                                   mut with: *const libc::c_char,
                                   mut text: *const libc::c_char,
                                   mut m: *mut regmatch_t, mut n: u_int) {
    ...
    while *cp as libc::c_int != '\u{0}' as i32 {
        if *cp as libc::c_int == '\\' as i32 {
            ...
            if *cp as libc::c_int >= '0' as i32 &&
                ...
                if i < n &&
                       (*m.offset(i as isize)).rm_so !=
                           (*m.offset(i as isize)).rm_eo {
                    regsub_copy(buf, len, text,
                                (*m.offset(i as isize)).rm_so as size_t,
                                (*m.offset(i as isize)).rm_eo as size_t);
                    current_block_5 = 16559507199688588974;
                } else { current_block_5 = 13109137661213826276; }
            } else { current_block_5 = 13109137661213826276; }
        } else { current_block_5 = 13109137661213826276; }
...
#[no_mangle]
pub unsafe extern "C" fn regsub(mut pattern: *const libc::c_char,
                                mut with: *const libc::c_char,
                                mut text: *const libc::c_char,
                                mut flags: libc::c_int) -> *mut libc::c_char {
    let mut r: regex_t =
    ...
    while start <= end {
        if regexec(&mut r, text.offset(start as isize),
                   (::std::mem::size_of::<[regmatch_t; 10]>() as
                        libc::c_ulong).wrapping_div(::std::mem::size_of::<regmatch_t>()
                                                        as libc::c_ulong),
                   m.as_mut_ptr(), 0 as libc::c_int) != 0 as libc::c_int {
            regsub_copy(&mut buf, &mut len as *mut ssize_t as *mut size_t,
                        text, start as size_t, end as size_t);
            break ;
        } else {
            /*
		 ...
		 */
            regsub_copy(&mut buf, &mut len as *mut ssize_t as *mut size_t,
                        text, last as size_t,
                        (m[0 as libc::c_int as usize].rm_so as libc::c_long +
                             start) as size_t);
            /*
		 ...
		 */
            if empty != 0 ||
                   start + m[0 as libc::c_int as usize].rm_so as libc::c_long
                       != last ||
                   m[0 as libc::c_int as usize].rm_so !=
                       m[0 as libc::c_int as usize].rm_eo {
                regsub_expand(&mut buf,
                              &mut len as *mut ssize_t as *mut size_t, with,
                              text.offset(start as isize), m.as_mut_ptr(),
                              (::std::mem::size_of::<[regmatch_t; 10]>() as
                                   libc::c_ulong).wrapping_div(::std::mem::size_of::<regmatch_t>()
                                                                   as
                                                                   libc::c_ulong)
                ...
                last =
                    start +
                        m[0 as libc::c_int as usize].rm_eo as libc::c_long;
                ...
                last =
                    start +
                        m[0 as libc::c_int as usize].rm_eo as libc::c_long;
                start +=
                    (m[0 as libc::c_int as usize].rm_eo + 1 as libc::c_int) as
                        libc::c_long;
                empty = 1 as libc::c_int
            }
            /* Stop now if anchored to start. */
            if !(*pattern as libc::c_int == '^' as i32) { continue ; }
            regsub_copy(&mut buf, &mut len as *mut ssize_t as *mut size_t,
                        text, start as size_t, end as size_t);
            break ;
        }
