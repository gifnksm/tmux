    ...
    unsafe {
        {
            let mut init =
                cmd_entry{name:
                          ...
                          usage:
                              b"[-Fhgru] [-t target-session] name [value]\x00"
                          ...
                          exec:
                              Some(cmd_set_environment_exec as
    ...
    if (*args).argc < 2 as libc::c_int {
        ...
        expand =
            format_single_from_target(item,
                                      *(*args).argv.offset(1 as libc::c_int as
                                                               isize));
        value = expand
    } else { value = *(*args).argv.offset(1 as libc::c_int as isize) }
    ...
    match current_block {
        10043043949733653460 => {
            if args_has(args, 'u' as i32 as u_char) != 0 {
                if !value.is_null() {
                    cmdq_error(item,
                               b"can\'t specify a value with -u\x00" as
                ...
                if !value.is_null() {
                    cmdq_error(item,
                               b"can\'t specify a value with -r\x00" as
                                   *const u8 as *const libc::c_char);
                    retval = CMD_RETURN_ERROR
                ...
                environ_set(env, name, 0x1 as libc::c_int,
                            b"%s\x00" as *const u8 as *const libc::c_char,
                            value);
            } else {
                environ_set(env, name, 0 as libc::c_int,
                            b"%s\x00" as *const u8 as *const libc::c_char,
                            value);
            }
        }
