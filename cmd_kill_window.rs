    ...
    unsafe {
        {
            let mut init =
                cmd_entry{name:
                              b"kill-window\x00" as *const u8 as
                                  *const libc::c_char,
                          alias:
                              b"killw\x00" as *const u8 as
                                  *const libc::c_char,
                          args:
                              {
                                  let mut init =
                          ...
                          usage:
                              b"[-a] [-t target-window]\x00" as *const u8 as
                                  *const libc::c_char,
                          source:
    ...
    unsafe {
        {
            let mut init =
                cmd_entry{name:
                              b"unlink-window\x00" as *const u8 as
                                  *const libc::c_char,
                          alias:
                              b"unlinkw\x00" as *const u8 as
                                  *const libc::c_char,
                          args:
                              {
                                  let mut init =
                          ...
                          usage:
                              b"[-k] [-t target-window]\x00" as *const u8 as
                                  *const libc::c_char,
                          source:
                              cmd_entry_flag{flag: 0,
                                             type_0: CMD_FIND_PANE,
 ...
 * Destroy window.
 */
unsafe extern "C" fn cmd_kill_window_exec(mut self_0: *mut cmd,
 ...
 -> cmd_retval {
    ...
    if cmd_get_entry(self_0) == &cmd_unlink_window_entry as *const cmd_entry {
        if args_has(args, 'k' as i32 as u_char) == 0 &&
               session_is_linked(s, w) == 0 {
            cmdq_error(item,
                       b"window only linked to one session\x00" as *const u8
    ...
    if args_has(args, 'a' as i32 as u_char) != 0 {
        ...
        loop 
            ...
            loop_0 =
                winlinks_RB_MINMAX(&mut (*s).windows, -(1 as libc::c_int));
            while !loop_0.is_null() {
                if (*loop_0).window != (*wl).window {
                    server_kill_window((*loop_0).window, 0 as libc::c_int);
                    found = found.wrapping_add(1);
                    break ;
                } else { loop_0 = winlinks_RB_NEXT(loop_0) }
            }
            if !(found != 0 as libc::c_int as libc::c_uint) { break ; }
        }
        /*
