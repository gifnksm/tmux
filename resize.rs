 ...
 */
#[no_mangle]
pub unsafe extern "C" fn resize_window(mut w: *mut window, mut sx: u_int,
    ...
    if zoomed != 0 { window_unzoom(w); }
    /* Resize the layout first. */
    layout_resize(w, sx, sy);
    /* Resize the window, it can be no smaller than the layout. */
    if sx < (*(*w).layout_root).sx { sx = (*(*w).layout_root).sx }
    if sy < (*(*w).layout_root).sy { sy = (*(*w).layout_root).sy }
    window_resize(w, sx, sy, xpixel, ypixel);
    log_debug(b"%s: @%u resized to %u,%u; layout %u,%u\x00" as *const u8 as
                  *const libc::c_char,
              (*::std::mem::transmute::<&[u8; 14],
                                        &[libc::c_char; 14]>(b"resize_window\x00")).as_ptr(),
              (*w).id, sx, sy, (*(*w).layout_root).sx,
              (*(*w).layout_root).sy);
    /* Restore the window zoom state. */
    if zoomed != 0 { window_zoom((*w).active); }
...
unsafe extern "C" fn ignore_client_size(mut c: *mut client) -> libc::c_int {
    ...
    if (*c).session.is_null() { return 1 as libc::c_int }
    if (*c).flags &
        ...
        while !loop_0.is_null() {
            if !(*loop_0).session.is_null() {
                if !((*loop_0).flags &
                    ...
                    if !(*loop_0).flags &
                           0x20000 as libc::c_int as libc::c_ulong != 0 {
                        return 1 as libc::c_int
                    }
                }
            }
...
unsafe extern "C" fn clients_calculate_size(mut type_0: libc::c_int,
 ...
 -> libc::c_int {
    ...
    if type_0 == 2 as libc::c_int { return 0 as libc::c_int }
    /*
    ...
    *xpixel = *ypixel;
	 ...
	 */
    if type_0 == 3 as libc::c_int { n = clients_with_window(w) }
    /* Loop over the clients and work out the size. */
    loop_0 = clients.tqh_first;
    while !loop_0.is_null() {
        if !(ignore_client_size(loop_0) != 0) {
            if !(skip_client.expect("non-null function pointer")(loop_0,
                                                                 type_0,
                                                                 current, s,
                                                                 w) != 0) {
                /*
		 ...
		 */
                    ...
                    if (*loop_0).tty.xpixel > *xpixel &&
                           (*loop_0).tty.ypixel > *ypixel {
    ...
    if type_0 == 0 as libc::c_int {
        return (*sx != 0 as libc::c_int as libc::c_uint &&
                    *sy != 0 as libc::c_int as libc::c_uint) as libc::c_int
    }
    return (*sx !=
                (2147483647 as libc::c_int as
                     libc::c_uint).wrapping_mul(2 as
...
unsafe extern "C" fn default_window_size_skip_client(mut loop_0: *mut client,
 ...
 -> libc::c_int {
	 ...
	 */
    if type_0 == 3 as libc::c_int { return 0 as libc::c_int }
    if !w.is_null() && session_has((*loop_0).session, w) == 0 {
        return 1 as libc::c_int
    }
    if w.is_null() && (*loop_0).session != s { return 1 as libc::c_int }
    return 0 as libc::c_int;
}
#[no_mangle]
pub unsafe extern "C" fn default_window_size(mut c: *mut client,
                                             mut s: *mut session,
                                             mut w: *mut window,
                                             mut sx: *mut u_int,
                                             mut sy: *mut u_int,
    ...
    if type_0 == -(1 as libc::c_int) {
        type_0 =
            options_get_number(global_w_options,
    ...
    if type_0 == 3 as libc::c_int {
        if !c.is_null() && ignore_client_size(c) == 0 {
            ...
            *ypixel = (*c).tty.ypixel;
            ...
            if w.is_null() { type_0 = 2 as libc::c_int }
	 ...
	 */
            if clients_calculate_size(type_0, 0 as libc::c_int, s, w,
                                      Some(default_window_size_skip_client as
...
unsafe extern "C" fn recalculate_size_skip_client(mut loop_0: *mut client,
                                                  mut type_0: libc::c_int,
                                                  mut current: libc::c_int,
                                                  mut s: *mut session,
                                                  mut w: *mut window)
 -> libc::c_int {
    ...
    return (session_has((*loop_0).session, w) == 0 as libc::c_int) as
               libc::c_int;
}
#[no_mangle]
pub unsafe extern "C" fn recalculate_size(mut w: *mut window,
                                          mut now: libc::c_int) {
    let mut sx: u_int = 0;
    let mut sy: u_int = 0;
    ...
    log_debug(b"%s: @%u is %u,%u\x00" as *const u8 as *const libc::c_char,
              (*::std::mem::transmute::<&[u8; 17],
                                        &[libc::c_char; 17]>(b"recalculate_size\x00")).as_ptr(),
              (*w).id, (*w).sx, (*w).sy);
    /*
    ...
    changed =
        clients_calculate_size(type_0, current, 0 as *mut session, w,
                               Some(recalculate_size_skip_client as
                                        unsafe extern "C" fn(_: *mut client,
                                                             _: libc::c_int,
                                                             _: libc::c_int,
                                                             _: *mut session,
                                                             _: *mut window)
                                            -> libc::c_int), &mut sx, &mut sy,
                               &mut xpixel, &mut ypixel);
    /*
	 ...
	 * size.
	 */
    if changed == 0 { tty_update_window_offset(w); return }
    /*
	 ...
	 * later.
	 */
    log_debug(b"%s: @%u new size %u,%u\x00" as *const u8 as
                  *const libc::c_char,
              (*::std::mem::transmute::<&[u8; 17],
                                        &[libc::c_char; 17]>(b"recalculate_size\x00")).as_ptr(),
              (*w).id, sx, sy);
    if now != 0 || type_0 == 2 as libc::c_int {
        resize_window(w, sx, sy, xpixel as libc::c_int,
                      ypixel as libc::c_int);
    } else {
        (*w).new_sx = sx;
        (*w).new_sy = sy;
        (*w).new_xpixel = xpixel;
...
pub unsafe extern "C" fn recalculate_sizes_now(mut now: libc::c_int) {
    ...
    while !w.is_null() { recalculate_size(w, now); w = windows_RB_NEXT(w) };
}
