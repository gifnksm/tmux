...
extern "C" {
    ...
    fn xasprintf(_: *mut *mut libc::c_char, _: *const libc::c_char, _: ...)
     -> libc::c_int;
    #[no_mangle]
    fn args_set(_: *mut args, _: u_char, _: *const libc::c_char);
    #[no_mangle]
    ...
    unsafe {
        {
            let mut init =
                cmd_entry{name:
                              b"find-window\x00" as *const u8 as
                                  *const libc::c_char,
                          alias:
                              b"findw\x00" as *const u8 as
                                  *const libc::c_char,
                          args:
                              {
                                  let mut init =
                          ...
                          usage:
                              b"[-CiNrTZ] [-t target-pane] match-string\x00"
 ...
 */
unsafe extern "C" fn cmd_find_window_exec(mut self_0: *mut cmd,
 ...
 -> cmd_retval {
    ...
    if C == 0 && N == 0 && T == 0 { T = 1 as libc::c_int; N = T; C = N }
    if C != 0 && N != 0 && T != 0 {
        xasprintf(&mut filter as *mut *mut libc::c_char,
                  b"#{||:#{C%s:%s},#{||:#{m%s:*%s*,#{window_name}},#{m%s:*%s*,#{pane_title}}}}\x00"
                      as *const u8 as *const libc::c_char, suffix, s, suffix,
                  s, suffix, s);
    } else if C != 0 && N != 0 {
        xasprintf(&mut filter as *mut *mut libc::c_char,
                  b"#{||:#{C%s:%s},#{m%s:*%s*,#{window_name}}}\x00" as
                      *const u8 as *const libc::c_char, suffix, s, suffix, s);
    } else if C != 0 && T != 0 {
        xasprintf(&mut filter as *mut *mut libc::c_char,
                  b"#{||:#{C%s:%s},#{m%s:*%s*,#{pane_title}}}\x00" as
                      *const u8 as *const libc::c_char, suffix, s, suffix, s);
    } else if N != 0 && T != 0 {
        xasprintf(&mut filter as *mut *mut libc::c_char,
                  b"#{||:#{m%s:*%s*,#{window_name}},#{m%s:*%s*,#{pane_title}}}\x00"
                      as *const u8 as *const libc::c_char, suffix, s, suffix,
                  s);
    } else if C != 0 {
        xasprintf(&mut filter as *mut *mut libc::c_char,
        ...
        xasprintf(&mut filter as *mut *mut libc::c_char,
                  b"#{m%s:*%s*,#{window_name}}\x00" as *const u8 as
                      *const libc::c_char, suffix, s);
    } else {
        xasprintf(&mut filter as *mut *mut libc::c_char,
                  b"#{m%s:*%s*,#{pane_title}}\x00" as *const u8 as
                      *const libc::c_char, suffix, s);
    }
    new_args =
        args_parse(b"\x00" as *const u8 as *const libc::c_char,
                   1 as libc::c_int, &mut argv);
    if args_has(args, 'Z' as i32 as u_char) != 0 {
        args_set(new_args, 'Z' as i32 as u_char, 0 as *const libc::c_char);
    }
    args_set(new_args, 'f' as i32 as u_char, filter);
    window_pane_set_mode(wp, 0 as *mut window_pane, &window_tree_mode, target,
                         new_args);
    args_free(new_args);
