...
pub unsafe extern "C" fn ibuf_open(mut len: size_t) -> *mut ibuf {
    ...
    if buf.is_null() { return 0 as *mut ibuf }
...
pub unsafe extern "C" fn ibuf_dynamic(mut len: size_t, mut max: size_t)
 -> *mut ibuf {
    let mut buf: *mut ibuf = 0 as *mut ibuf;
    if max < len { return 0 as *mut ibuf }
    buf = ibuf_open(len);
    if buf.is_null() { return 0 as *mut ibuf }
    if max > 0 as libc::c_int as libc::c_ulong { (*buf).max = max }
    return buf;
}
 ...
 -> libc::c_int {
    ...
    b =
        recallocarray((*buf).buf as *mut libc::c_void, (*buf).size,
                      ...
                      1 as libc::c_int as size_t) as *mut libc::c_uchar;
...
pub unsafe extern "C" fn ibuf_seek(mut buf: *mut ibuf, mut pos: size_t,
    ...
    if pos.wrapping_add(len) > (*buf).wpos { return 0 as *mut libc::c_void }
...
pub unsafe extern "C" fn ibuf_write(mut msgbuf: *mut msgbuf) -> libc::c_int {
    ...
    let mut n: ssize_t = 0;
    memset(&mut iov as *mut [iovec; 1024] as *mut libc::c_void,
           0 as libc::c_int,
           ::std::mem::size_of::<[iovec; 1024]>() as libc::c_ulong);
    buf = (*msgbuf).bufs.tqh_first;
    while !buf.is_null() {
        if i >= 1024 as libc::c_int as libc::c_uint { break ; }
...
#[no_mangle]
pub unsafe extern "C" fn msgbuf_drain(mut msgbuf: *mut msgbuf,
                                      mut n: size_t) {
    let mut buf: *mut ibuf = 0 as *mut ibuf;
...
pub unsafe extern "C" fn msgbuf_write(mut msgbuf: *mut msgbuf)
 -> libc::c_int {
    ...
    memset(&mut cmsgbuf as *mut C2RustUnnamed_2 as *mut libc::c_void,
           0 as libc::c_int,
    ...
    while !buf.is_null() {
        ...
        iov[i as usize].iov_len = (*buf).wpos.wrapping_sub((*buf).rpos);
        i = i.wrapping_add(1);
        if (*buf).fd != -(1 as libc::c_int) { break ; }
        buf = (*buf).entry.tqe_next
    }
...
unsafe extern "C" fn ibuf_enqueue(mut msgbuf: *mut msgbuf,
                                  mut buf: *mut ibuf) {
...
unsafe extern "C" fn ibuf_dequeue(mut msgbuf: *mut msgbuf,
                                  mut buf: *mut ibuf) {
    ...
    *(*buf).entry.tqe_prev = (*buf).entry.tqe_next;
    if (*buf).fd != -(1 as libc::c_int) { close((*buf).fd); }
