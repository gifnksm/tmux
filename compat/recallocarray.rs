...
pub unsafe extern "C" fn recallocarray(mut ptr: *mut libc::c_void,
 ...
 -> *mut libc::c_void {
    ...
    if ptr.is_null() { return calloc(newnmemb, size) }
    if (newnmemb >=
           ...
           (18446744073709551615 as libc::c_ulong).wrapping_div(newnmemb) <
               size {
    ...
    if (oldnmemb >=
           ...
           (18446744073709551615 as libc::c_ulong).wrapping_div(oldnmemb) <
               size {
    ...
    if newsize <= oldsize {
        ...
        if d < oldsize.wrapping_div(2 as libc::c_int as libc::c_ulong) &&
            ...
            memset((ptr as *mut libc::c_char).offset(newsize as isize) as
                       *mut libc::c_void, 0 as libc::c_int, d);
            return ptr
        }
    }
    newptr = malloc(newsize);
    if newptr.is_null() { return 0 as *mut libc::c_void }
    if newsize > oldsize {
        memcpy(newptr, ptr, oldsize);
        memset((newptr as *mut libc::c_char).offset(oldsize as isize) as
