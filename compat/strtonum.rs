...
pub unsafe extern "C" fn strtonum(mut numstr: *const libc::c_char,
 ...
 -> libc::c_longlong {
    ...
    let mut ev: [errval; 4] =
        [{
             let mut init =
                 errval{errstr: 0 as *const libc::c_char,
                        err: 0 as libc::c_int,};
             init
         },
         {
         ...
         },
         {
             let mut init =
                 errval{errstr:
                            b"too small\x00" as *const u8 as
                                *const libc::c_char,
         ...
         },
         {
             let mut init =
                 errval{errstr:
                            b"too large\x00" as *const u8 as
                                *const libc::c_char,
                        err: 34 as libc::c_int,};
             init
         }];
    ...
    *__errno_location() = 0 as libc::c_int;
    if minval > maxval {
        ...
        if numstr == ep as *const libc::c_char ||
    ...
    if !errstrp.is_null() { *errstrp = ev[error as usize].errstr }
    *__errno_location() = ev[error as usize].err;
    if error != 0 { ll = 0 as libc::c_int as libc::c_longlong }
    return ll;
}
