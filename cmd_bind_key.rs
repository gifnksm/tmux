...
extern "C" {
    ...
    #[no_mangle]
    fn key_bindings_add(_: *const libc::c_char, _: key_code,
    ...
    unsafe {
        {
            let mut init =
                cmd_entry{name:
                          ...
                          usage:
                              b"[-nr] [-T key-table] [-N note] key [command [arguments]]\x00"
 ...
 */
unsafe extern "C" fn cmd_bind_key_exec(mut self_0: *mut cmd,
 ...
 -> cmd_retval {
    ...
    if argc != 1 as libc::c_int {
        if argc == 2 as libc::c_int {
            pr =
                cmd_parse_from_string(*argv.offset(1 as libc::c_int as isize),
                                      0 as *mut cmd_parse_input)
        } else {
            pr =
                cmd_parse_from_arguments(argc - 1 as libc::c_int,
        ...
        match (*pr).status as libc::c_uint {
            0 => {
                cmdq_error(item,
                           b"empty command\x00" as *const u8 as
                               *const libc::c_char);
                return CMD_RETURN_ERROR
            }
            1 => {
        ...
        key_bindings_add(tablename, key, note, repeat, 0 as *mut cmd_list);
    }
