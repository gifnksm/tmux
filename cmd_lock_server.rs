    ...
    unsafe {
        {
            let mut init =
                cmd_entry{name:
                              b"lock-server\x00" as *const u8 as
                                  *const libc::c_char,
                          alias:
    ...
    unsafe {
        {
            let mut init =
                cmd_entry{name:
                              b"lock-session\x00" as *const u8 as
                                  *const libc::c_char,
                          alias:
                              b"locks\x00" as *const u8 as
                                  *const libc::c_char,
                          args:
                              {
                                  let mut init =
    ...
    unsafe {
        {
            let mut init =
                cmd_entry{name:
                              b"lock-client\x00" as *const u8 as
                                  *const libc::c_char,
                          alias:
                              b"lockc\x00" as *const u8 as
                                  *const libc::c_char,
                          args:
                              {
                                  let mut init =
 ...
 * Lock commands.
 */
unsafe extern "C" fn cmd_lock_server_exec(mut self_0: *mut cmd,
                                          mut item: *mut cmdq_item)
 -> cmd_retval {
    let mut target: *mut cmd_find_state = cmdq_get_target(item);
    let mut tc: *mut client = cmdq_get_target_client(item);
    if cmd_get_entry(self_0) == &cmd_lock_server_entry as *const cmd_entry {
        server_lock();
    } else if cmd_get_entry(self_0) ==
                  &cmd_lock_session_entry as *const cmd_entry {
        server_lock_session((*target).s);
    } else { server_lock_client(tc); }
    recalculate_sizes();
