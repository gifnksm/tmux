    ...
    unsafe {
        {
            let mut init =
                cmd_entry{name:
                              b"list-windows\x00" as *const u8 as
                                  *const libc::c_char,
                          alias:
                              b"lsw\x00" as *const u8 as *const libc::c_char,
                          args:
                              {
                                  let mut init =
                          ...
                          usage:
                              b"[-a] [-F format] [-f filter] [-t target-session]\x00"
...
unsafe extern "C" fn cmd_list_windows_exec(mut self_0: *mut cmd,
...
unsafe extern "C" fn cmd_list_windows_server(mut self_0: *mut cmd,
                                             mut item: *mut cmdq_item) {
...
unsafe extern "C" fn cmd_list_windows_session(mut self_0: *mut cmd,
                                              mut s: *mut session,
                                              mut item: *mut cmdq_item,
    ...
    if template.is_null() {
        match type_0 {
            0 => {
                template =
                    b"#{window_index}: #{window_name}#{window_flags} (#{window_panes} panes) [#{window_width}x#{window_height}] [layout #{window_layout}] #{window_id}#{?window_active, (active),}\x00"
                        as *const u8 as *const libc::c_char
            }
            1 => {
                template =
                    b"#{session_name}:#{window_index}: #{window_name}#{window_flags} (#{window_panes} panes) [#{window_width}x#{window_height}] \x00"
                        as *const u8 as *const libc::c_char
            }
            _ => { }
        }
